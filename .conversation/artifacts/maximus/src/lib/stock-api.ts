...
export interface StockMovement { id: string; companyId: string; productId: string; warehouseId: string; destinationWarehouseId: string | null; locationId: string | null; type: StockMovementType; quantity: number; purchasePrice: number; reason: string; movementDate: string; userName: string; reference: string; comment: string; status: string; }
...
export interface StockBootstrap { products: StockProduct[]; warehouses: StockWarehouse[]; locations: StockLocation[]; suppliers: StockSupplier[]; balances: StockBalance[]; movements: StockMovement[]; inventories: StockInventory[]; inventoryLines: StockInventoryLine[]; }
