...
**How to apply:** Utiliser une valeur équivalente à `location.split('?')[0]` pour sélectionner la vue, tout en laissant le composant concerné lire les paramètres nécessaires. Avec Wouter 3, `useLocation()` renvoie le pathname sans la query string : utiliser `useSearch()` pour les paramètres de retour, filtres ou onglets. Retirer aussi les slashs finaux avant la comparaison, car une actualisation peut préserver une URL profonde sous la forme `/route/`. Pour les fonctionnalités Paie, transmettre explicitement `feature` jusqu’au composant : reconnaître la route ne suffit pas si la page ignore le paramètre.