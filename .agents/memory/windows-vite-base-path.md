...
**Why:** Un build local exécuté via le parcours shell a produit des URLs `/Program Files/Git/assets/` dans le HTML, signature de la conversion MSYS de `/`. Le routeur SPA renvoyait alors du HTML pour les modules JavaScript, entraînant une page blanche malgré des connexions PHP acceptées. Le fonctionnement après reconstruction native PowerShell et remise en écoute du serveur local a été confirmé par l’utilisateur ; ne pas attribuer cette erreur MIME à l’antivirus sans autre preuve.
