...
Sur un hôte qui n’est ni le domaine principal MAXIMUS ni un domaine de prévisualisation, la route `/` doit être traitée comme une boutique publique avant tout rendu de la connexion MAXIMUS. La route `/connexion` est l’exception explicite : elle peut résoudre l’identité visuelle de l’entreprise liée au domaine configuré uniquement si l’administration MAXIMUS a activé cette option, puis authentifier uniquement un compte de cette entreprise. Sinon, la connexion générale reste disponible. La détection de la vitrine peut ensuite charger ses données de façon asynchrone.
