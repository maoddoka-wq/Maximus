  ...
  if (run.action?.kind === 'openUrl') {
    ...
    return (
      <a
        ...
        data-sdm-text-run=""
        style={{ ...style, cursor: moveCursor ? 'move' : undefined }}
  ...
  if (run.action) {
    ...
    return (
      <span
        ...
        data-sdm-text-run=""
        style={{ ...style, cursor: moveCursor ? 'move' : 'pointer' }}
        onClick={(event) => {
  ...
  return (
    <div
      ...
      style={{
        ...
        overflow: body.overflow === 'clip' ? 'hidden' : 'visible',
        padding: inset
...
function ElementInner({
  ...
  switch (element.type) {
    ...
    case 'shape': {
      ...
      const strokeWidth = element.stroke
        ? element.stroke.widthPt * SDM_POINT_TO_UNIT
        : 0;
...
export function SdmElementView({
  ...
  const showMoveCursor = moveCursor && textEditor === undefined;
  ...
  return (
    <div
      ...
      style={{
        ...frameStyle(element),
        cursor: showMoveCursor ? 'move' : element.action ? 'pointer' : undefined,
      }}
      onClick={handleClick}
    >
      <ElementInner
