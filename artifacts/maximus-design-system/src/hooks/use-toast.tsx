...
const addToRemoveQueue = (id: string) => { if (toastTimeouts.has(id)) return; const timeout = setTimeout(() => { toastTimeouts.delete(id); dispatch({ type: 'REMOVE_TOAST', toastId: id }); }, TOAST_REMOVE_DELAY); toastTimeouts.set(id, timeout); };
...
export function toast({ ...props }: Toast) { const id = genId(); const update = (next: ToasterToast) => dispatch({ type: 'UPDATE_TOAST', toast: { ...next, id } }); const dismiss = () => dispatch({ type: 'DISMISS_TOAST', toastId: id }); dispatch({ type: 'ADD_TOAST', toast: { ...props, id, open: true, onOpenChange: (open) => { if (!open) dismiss(); } } }); return { id, dismiss, update }; }
export type AppToastKind = 'success' | 'error' | 'info' | 'warning';
export function showAppToast(message: string, kind: AppToastKind = 'info') { const titles: Record<AppToastKind, string> = { success: 'Succès', error: 'Erreur', info: 'Information', warning: 'Attention' }; return toast({ title: titles[kind], description: message, variant: kind === 'error' ? 'destructive' : kind }); }
