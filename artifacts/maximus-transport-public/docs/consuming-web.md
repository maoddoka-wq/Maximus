# Consuming MAXIMUS Transport Public in web apps

Read `artifacts/maximus-transport-public/docs/AGENTS.md` first. This guide covers
...
`artifacts/maximus-transport-public/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/maximus-transport-public/styles.css";
```
...
`node_modules/@workspace/maximus-transport-public/src/components` to `content`.
...
import {
  toast,
  useToast,
} from "@workspace/maximus-transport-public/hooks/use-toast";
```
...
`@workspace/maximus-transport-public/components/ui/button`. Run the app's typecheck
