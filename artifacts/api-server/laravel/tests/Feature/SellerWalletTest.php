...
class SellerWalletTest extends TestCase
{
    ...
    public function test_public_payment_uses_the_customer_selected_provider(): void
    {
        config([
            'services.diamanopay.access_token' => 'static-test-token',
            'services.diamanopay.provider' => 'WAVE',
        ...
        foreach ([
            ...
            $this->postJson("/api/shop/{$choice['slug']}/orders/{$choice['order']}/payment", [
    ...
    {
        $this->createStore('kora', 'kora-choice-invalid');
        $this->createOrder('order-choice-invalid', 'kora', 2500, '');

        $this->postJson('/api/shop/kora-choice-invalid/orders/order-choice-invalid/payment', [
            'provider' => 'UNKNOWN',
        ])->assertUnprocessable()
