...
class OnboardingTest extends TestCase
{
    ...
    {
        config()->set('services.replit_ai.key', null);
    ...
    public function test_it_rejects_an_invalid_ai_json_response_without_creating_a_draft(): void
    {
        Http::fake([
            'https://openai.test/v1/chat/completions' => Http::response([
                'choices' => [[
                    'message' => ['content' => 'Voici une proposition non structurée.'],
                ]],
            ], 200),
        ]);
        config()->set('services.replit_ai.key', 'test-replit-ai-key');
    ...
    public function test_it_falls_back_when_replit_ai_reaches_a_limit(): void
    {
        Http::fake([
        ...
        $this->postJson('/api/onboarding/drafts', [
            ...
            ->assertStatus(503)
            ->assertJsonPath('code', 'ONBOARDING_ANALYSIS_UNAVAILABLE')
            ->assertJsonPath('error', 'L’analyse intelligente est momentanément indisponible. Utilisez la configuration manuelle.');
    }
}