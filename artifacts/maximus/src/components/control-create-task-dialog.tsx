...
import { modules, type Company, type ControlTaskPriority, type Employee, type ModuleId } from '@/lib/store';
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
    <div className="card-surface w-full max-w-lg rounded-2xl border p-6 shadow-2xl">
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-2"><button type="button" onClick={onClose} className="rounded-lg border px-4 py-2.5 text-xs font-bold">Annuler</button><button type="button" disabled={!form.title.trim() || !targetCompanyId || !assigneeEmployeeId} onClick={onSubmit} className="flex items-center gap-2 rounded-lg bg-[hsl(var(--primary))] px-4 py-2.5 text-xs font-bold text-[hsl(var(--primary-foreground))] disabled:cursor-not-allowed disabled:opacity-50"><ArrowRight size={15} /> Créer et tracer</button></div>
      </div>
    </div>
  </div>;
}