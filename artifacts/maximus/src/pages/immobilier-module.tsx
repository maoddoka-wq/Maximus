...
export default function ImmobilierModulePage({ companyId, canCreate = true, canModify = true, featurePermissions, activeFeatureId = 'dashboard', preview = false }: { companyId: string; canCreate?: boolean; canModify?: boolean; featurePermissions?: Partial<Record<string, string[]>>; activeFeatureId?: string; preview?: boolean }) {
  ...
  const propertyView = activeFeatureId === 'biens';
  const leadView = activeFeatureId === 'prospects' || activeFeatureId === 'visites';
  const listingTitle = propertyView ? 'Biens immobiliers' : 'Annonces immobilières';
  const listingDescription = propertyView
    ? 'Gérez les biens disponibles et leurs caractéristiques avant de les publier dans une annonce.'
    : 'Publiez les biens disponibles et suivez leur statut de diffusion sur votre vitrine.';
  ...
  useEffect(() => { setTab(leadView ? 'leads' : 'listings'); }, [activeFeatureId, leadView]);
  ...
  return <div className="space-y-5 fade-up">
    <section className="rounded-2xl border border-[hsl(var(--primary)/.25)] bg-[hsl(var(--primary)/.06)] p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div><p className="text-xs font-bold uppercase tracking-[.18em] text-[hsl(var(--primary))]">Immobilier</p><h1 className="mt-2 text-2xl font-bold tracking-[-.04em]">{leadView ? activeFeatureId === 'visites' ? 'Visites immobilières' : 'Prospects immobiliers' : listingTitle}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[hsl(var(--muted-foreground))]">{leadView ? 'Suivez les demandes reçues et transformez les contacts en visites et opportunités.' : listingDescription}</p></div>
    ...
    <div className="flex gap-2 border-b"><button type="button" onClick={() => setTab('listings')} className={`px-4 py-3 text-sm font-bold ${tab === 'listings' ? 'border-b-2 border-[hsl(var(--primary))] text-[hsl(var(--primary))]' : 'text-[hsl(var(--muted-foreground))]'}`}>{propertyView ? 'Biens' : 'Annonces'}</button><button type="button" onClick={() => setTab('leads')} className={`px-4 py-3 text-sm font-bold ${tab === 'leads' ? 'border-b-2 border-[hsl(var(--primary))] text-[hsl(var(--primary))]' : 'text-[hsl(var(--muted-foreground))]'}`}>Prospects et visites</button></div>
