...
export default function TransportModulePage({
  ...
  const addPreviewVehicle = (input: CreateVehicleInput) => {
    if (!data) return;
    const vehicle: Vehicle = { ...input, id: `preview-vehicle-${Date.now()}`, companyId, driverId: input.driverId };
...
function VehicleDialog({ busy, drivers, vehicles, onClose, onSubmit }: { busy: boolean; drivers: Driver[]; vehicles: Vehicle[]; onClose: () => void; onSubmit: (input: CreateVehicleInput) => void }) {
  ...
  const [form, setForm] = useState<CreateVehicleInput>({ registration: '', model: '', vehicleType: 'Berline', driverId: '', status: 'AVAILABLE' });
  const submit = (event: FormEvent) => { event.preventDefault(); if (form.registration.trim() && form.model.trim() && form.vehicleType.trim() && form.driverId) onSubmit({ ...form, registration: form.registration.trim().toUpperCase(), model: form.model.trim(), vehicleType: form.vehicleType.trim() }); };
  return <DialogShell title="Ajouter un véhicule" description="Enregistrez une voiture et rattachez-la à son chauffeur actif." onClose={onClose}><form onSubmit={submit} className="mt-6 space-y-4"><div className="grid gap-4 sm:grid-cols-2"><Field label="Immatriculation"><input required autoFocus value={form.registration} onChange={event => setForm({ ...form, registration: event.target.value })} className={`${inputClass} uppercase`} placeholder="DK-1234-AB" /></Field><Field label="Modèle"><input required value={form.model} onChange={event => setForm({ ...form, model: event.target.value })} className={inputClass} placeholder="Toyota Corolla" /></Field></div><Field label="Chauffeur rattaché"><select required value={form.driverId} onChange={event => setForm({ ...form, driverId: event.target.value })} className={inputClass}><option value="">Sélectionner un chauffeur…</option>{eligibleDrivers.map(driver => <option key={driver.id} value={driver.id}>{driver.name}</option>)}</select></Field><Field label="Type de véhicule"><input required value={form.vehicleType} onChange={event => setForm({ ...form, vehicleType: event.target.value })} className={inputClass} placeholder="Berline, van, premium…" /></Field><Field label="État initial"><select value={form.status} onChange={event => setForm({ ...form, status: event.target.value as VehicleStatus })} className={inputClass}><option value="AVAILABLE">Disponible</option><option value="MAINTENANCE">Maintenance</option><option value="ON_TRIP">En course</option></select></Field>{eligibleDrivers.length === 0 && <p className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs font-semibold text-amber-800">Qualifiez d’abord un chauffeur actif dans Transport avant d’ajouter son véhicule.</p>}<DialogActions busy={busy} disabled={eligibleDrivers.length === 0 || !form.driverId || !form.registration.trim() || !form.model.trim()} onClose={onClose} label="Enregistrer le véhicule" /></form></DialogShell>;
}
