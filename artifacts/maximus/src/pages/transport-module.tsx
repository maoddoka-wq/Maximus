...
export default function TransportModulePage({
  ...
  const removeDriver = (driver: Driver) => {
    if (!canModifyDrivers || pendingAction) return;
    if (!window.confirm(`Supprimer le chauffeur ${driver.name} ? Cette action est définitive.`)) return;
    if (preview) {
      setData(current => current ? {
        ...current,
        ...
        metrics: {
          ...current.metrics,
          activeDrivers: driver.status === 'ACTIVE'
            ? Math.max(0, current.metrics.activeDrivers - 1)
            : current.metrics.activeDrivers,
        },
      } : current);
      showAppToast('Chauffeur supprimé de l’aperçu local.', 'success');
      return;
    }
    void run(() => api.deleteDriver(driver.id), 'Chauffeur supprimé.');
  };
    ...
    >
      ...
      {visibleTabs.length === 0 ? <EmptyState icon={ShieldCheck} title="Aucune fonctionnalité disponible" text="Votre rôle n’a pas encore reçu de fonctionnalité pour cet espace." /> : <>
        ...
        {tab === 'courses' && <TripsPanel data={data} drivers={data.drivers} vehicles={data.vehicles} canCreate={canOperateTrips || canCreateTrips} canModify={canModifyTrips} onCreate={() => setDialog('trip')} onStatusChange={updateStatus} onAssign={assignTrip} />}
         {tab === 'chauffeurs' && <><DriversPanel drivers={data.drivers} canCreate={canCreateDrivers} canModify={canModifyDrivers} onCreate={() => setDialog('driver')} onDelete={removeDriver} /><DriverLocationPanel driver={currentDriver} active={locationActive} error={locationError} onAvailabilityChange={updateAvailability} fullscreen={driverFullscreen || driverFullscreenFallback} onFullscreenToggle={() => void toggleDriverFullscreen()} /></>}
      ...
      {dialog === 'vehicle' && <VehicleDialog busy={Boolean(pendingAction)} vehicle={editingVehicle} drivers={data.drivers} vehicles={data.vehicles} onClose={() => { setDialog(null); setEditingVehicle(null); }} onSubmit={input => {
        if (editingVehicle) {
          ...
          if (preview) {
            addPreviewVehicle({ ...input, imageData: input.imageData as string });
          } else {
