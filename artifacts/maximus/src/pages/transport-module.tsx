  ...
  return (
    <div className="space-y-5" data-testid="transport-module" aria-busy={Boolean(pendingAction)}>
      ...
      {visibleTabs.length === 0 ? <EmptyState icon={ShieldCheck} title="Aucune fonctionnalité disponible" text="Votre rôle n’a pas encore reçu de fonctionnalité pour cet espace." /> : <>
        {tab === 'overview' && <><Overview data={data} onTab={setTab} />{currentDriver && <DriverLocationPanel driver={currentDriver} active={locationActive} error={locationError} />}{canOperateTrips && <DriverRequestCard trip={data.trips.find(item => item.status === 'OFFERED') ?? null} vehicle={data.vehicles.find(item => item.id === data.trips.find(trip => trip.status === 'OFFERED')?.vehicleId) ?? null} onAccept={trip => updateStatus(trip, 'ASSIGNED')} />}{canOperateTrips && <DriverTripTracking trip={data.trips.find(item => ['ASSIGNED', 'IN_PROGRESS'].includes(item.status)) ?? null} driver={currentDriver} vehicle={data.vehicles.find(item => item.id === data.trips.find(trip => ['ASSIGNED', 'IN_PROGRESS'].includes(trip.status))?.vehicleId) ?? null} />}</>}
...
function DriverRequestCard({ trip, vehicle, onAccept }: { trip: Trip | null; vehicle: Vehicle | null; onAccept: (trip: Trip) => void }) {
  if (!trip) return null;
  const routeUrl = trip.pickupLatitude !== null && trip.pickupLatitude !== undefined && trip.pickupLongitude !== null && trip.pickupLongitude !== undefined
    ? `https://www.google.com/maps/dir/?api=1&destination=${trip.pickupLatitude},${trip.pickupLongitude}`
    : null;
  return <section className="card-surface border-2 border-[hsl(var(--primary)/.35)] bg-[hsl(var(--primary)/.05)] p-5 shadow-sm"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex min-w-0 items-start gap-3">{vehicle?.imageUrl && <img src={vehicle.imageUrl} alt={`Photo de ${vehicle.model}`} className="h-16 w-20 shrink-0 rounded-xl border bg-white object-cover" />}<div className="min-w-0"><p className="mono text-[10px] font-bold uppercase tracking-[.16em] text-[hsl(var(--primary))]">Nouvelle demande détectée</p><h2 className="mt-1 text-lg font-black">Course à valider</h2><p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">{trip.pickup} <span className="mx-1">→</span> {trip.destination}</p><p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">Passager : {trip.passengerName} · {money(trip.fare)}</p>{vehicle && <p className="mt-2 text-xs font-bold">{vehicle.model} · {vehicle.registration}</p>}{trip.matchedDistanceKm !== null && trip.matchedDistanceKm !== undefined && <p className="mt-1 text-xs font-semibold text-sky-700">Chauffeur à {formatDistance(trip.matchedDistanceKm)} du client</p>}</div></div><div className="flex flex-wrap gap-2 lg:justify-end"><a href={routeUrl ?? '#'} onClick={event => { if (!routeUrl) event.preventDefault(); }} target={routeUrl ? '_blank' : undefined} rel="noreferrer" className={`inline-flex items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-bold ${routeUrl ? 'text-sky-700 hover:bg-sky-50' : 'cursor-not-allowed text-slate-400'}`}><Navigation size={16} />Itinéraire client</a><button type="button" onClick={() => onAccept(trip)} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-[hsl(var(--primary))] px-5 py-3 text-sm font-black text-[hsl(var(--primary-foreground))] shadow-sm"><CheckCircle2 size={17} />Valider la course</button></div></div></section>;
}
...
function DriverTripTracking({ trip, driver, vehicle }: { trip: Trip | null; driver: Driver | null; vehicle: Vehicle | null }) {
  ...
  const remainingDistance = trip.pickupLatitude !== null && trip.pickupLatitude !== undefined && trip.pickupLongitude !== null && trip.pickupLongitude !== undefined && driver.latitude !== null && driver.longitude !== null
    ? distanceInKm(driver.latitude, driver.longitude, trip.pickupLatitude, trip.pickupLongitude)
    : null;
  const routeUrl = trip.pickupLatitude !== null && trip.pickupLatitude !== undefined && trip.pickupLongitude !== null && trip.pickupLongitude !== undefined
    ? `https://www.google.com/maps/dir/?api=1&destination=${trip.pickupLatitude},${trip.pickupLongitude}`
    : null;
  return <section className="card-surface overflow-hidden p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="flex min-w-0 items-start gap-3">{vehicle?.imageUrl && <img src={vehicle.imageUrl} alt={`Photo de ${vehicle.model}`} className="h-14 w-20 shrink-0 rounded-xl border bg-white object-cover" />}<div><p className="mono text-[10px] font-bold uppercase tracking-[.16em] text-[hsl(var(--muted-foreground))]">Navigation chauffeur</p><h2 className="mt-1 text-lg font-black">Itinéraire vers le client</h2><p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">{trip.pickup} <span className="mx-1">→</span> {trip.destination}</p>{remainingDistance !== null ? <p className="mt-2 text-sm font-black text-sky-700">{formatDistance(remainingDistance)} jusqu’au point de prise en charge</p> : <p className="mt-2 text-xs font-semibold text-amber-700">Position GPS ou coordonnées client en attente</p>}</div></div><a href={routeUrl ?? '#'} onClick={event => { if (!routeUrl) event.preventDefault(); }} target={routeUrl ? '_blank' : undefined} rel="noreferrer" className={`inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-bold ${routeUrl ? 'text-sky-700 hover:bg-sky-50' : 'cursor-not-allowed text-slate-400'}`}><Navigation size={16} />Ouvrir l’itinéraire</a></div><TrackingSurface pickupLatitude={trip.pickupLatitude ?? null} pickupLongitude={trip.pickupLongitude ?? null} driverLatitude={driver.latitude} driverLongitude={driver.longitude} driverLabel="Votre véhicule" clientLabel="Client" /><p className="mt-3 text-xs text-[hsl(var(--muted-foreground))]">Distance recalculée avec votre dernière position GPS. Actualisation automatique toutes les 10 secondes.</p></section>;
}
