...
export default function PublicShopPage({ slug, domain = false }: { slug?: string; domain?: boolean }) {
  ...
  const { store, products, rentals } = data;
  ...
  return <section className="mx-auto max-w-5xl">
    ...
    <div className="mt-5 grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
      <div className="rounded-3xl p-6 text-white shadow-lg sm:p-8" style={{ background: `linear-gradient(145deg, ${store.accentColor}, ${store.primaryColor})` }}><span className="relative mono text-[10px] font-bold uppercase tracking-[.2em] text-white/70">Location & réservation</span><h1 className="mt-4 text-3xl font-bold tracking-[-.05em]">Des solutions prêtes pour vos projets.</h1><p className="mt-4 text-sm leading-7 text-white/75">Maison, bâche, voiture ou équipement : découvrez les offres publiées comme locations, sans les confondre avec les articles à vendre.</p><div className="mt-8 grid grid-cols-3 gap-2">{[['Maison', Home], ['Bâche', Sparkles], ['Voiture', Truck]].map(([label, Icon]) => <div key={label as string} className="rounded-xl bg-white/10 p-3 text-center"><Icon size={18} className="mx-auto" /><span className="mt-2 block text-[11px] font-bold">{label as string}</span></div>)}</div></div>
       <div className="rounded-3xl border bg-[hsl(var(--card))] p-5 shadow-sm sm:p-7"><div className="flex items-start justify-between gap-3"><div><span className="mono text-[10px] font-bold uppercase tracking-[.18em]" style={{ color: store.primaryColor }}>Offres disponibles</span><h2 className="mt-2 text-2xl font-bold">Trouvez votre prochaine location</h2></div><Home size={22} style={{ color: store.accentColor }} /></div>{rentals.length === 0 ? <div className="mt-8 rounded-2xl border border-dashed p-8 text-center"><p className="text-sm font-semibold">Les offres de location arrivent bientôt.</p><p className="mt-2 text-xs leading-5 text-[hsl(var(--muted-foreground))]">Aucune location publiée n’est disponible dans cette boutique.</p></div> : <div className="mt-6 grid gap-3 sm:grid-cols-2">{rentals.map(rental => <article key={rental.name} className="overflow-hidden rounded-2xl border bg-white"><div className="flex aspect-[16/9] items-center justify-center bg-[hsl(var(--muted)/.5)]"><Home size={28} className="text-[hsl(var(--muted-foreground))]" /></div><div className="p-4"><p className="text-[10px] font-bold uppercase tracking-[.14em]" style={{ color: store.primaryColor }}>LOCATION · {rental.billingUnit}</p><h3 className="mt-2 font-bold">{rental.name}</h3><p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">{rental.category}</p><p className="mt-2 text-sm font-bold" style={{ color: store.accentColor }}>{money(rental.price, store.currency)} <span className="text-xs font-medium text-[hsl(var(--muted-foreground))]">/ {rental.billingUnit === 'MOIS' ? 'mois' : rental.billingUnit === 'SEMAINE' ? 'semaine' : 'jour'}</span></p><p className="mt-2 line-clamp-2 text-xs leading-5 text-[hsl(var(--muted-foreground))]">{rental.description || 'Une offre proposée par cette boutique.'}</p><span className={`mt-4 inline-flex rounded-full px-3 py-1 text-[11px] font-bold ${rental.isAvailable ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>{rental.isAvailable ? `${rental.availability} disponible${rental.availability > 1 ? 's' : ''}` : 'Indisponible'}</span></div></article>)}</div>}</div>
    </div>
  </section>;
}
