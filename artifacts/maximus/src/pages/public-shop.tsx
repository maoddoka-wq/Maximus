  ...
  return <section className="mx-auto w-full max-w-3xl overflow-hidden border border-[#d9dcd6] bg-[#f2f3ef] text-[#172235] shadow-sm sm:max-w-5xl">
    ...
    <div className="px-4 pb-24 pt-5 sm:px-8">
      ...
      {formOpen && !trip && <form id="transport-booking-form" onSubmit={submit} className="mt-5 space-y-3">
        ...
        <details className="border border-[#d9dcd6] bg-[#f8f9f5] px-4 py-3"><summary className="flex cursor-pointer list-none items-center justify-between text-left text-[12px] font-bold">Coordonnées passager <ChevronDown size={16} className="text-[#687173]" /></summary><div className="grid gap-3 border-t border-[#e1e3de] pt-3"><label className="text-[10px] font-bold uppercase tracking-[.1em] text-[#7b8182]">Téléphone<input required type="tel" value={form.passengerPhone} onChange={event => setForm(current => ({ ...current, passengerPhone: event.target.value }))} className="mt-1.5 w-full border border-[#cfd3cd] bg-[#fffefa] px-3 py-2.5 text-[12px] font-medium normal-case tracking-normal outline-none" placeholder="+221 77 000 00 00" /></label><label className="text-[10px] font-bold uppercase tracking-[.1em] text-[#7b8182]">Prénom<input value={form.passengerName === 'Client Taxi' ? '' : form.passengerName} onChange={event => setForm(current => ({ ...current, passengerName: event.target.value || 'Client Taxi' }))} className="mt-1.5 w-full border border-[#cfd3cd] bg-[#fffefa] px-3 py-2.5 text-[12px] font-medium normal-case tracking-normal outline-none" placeholder="Ex. Awa" /></label></div></details>
