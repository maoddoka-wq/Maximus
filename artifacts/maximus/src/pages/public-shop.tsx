...
type PaymentSummary = Pick<PublicPaymentStatus, 'reference' | 'total' | 'paymentStatus' | 'failureReason' | 'digitalDownloads'>;
...
export default function PublicShopPage({ slug, domain = false, clientApp = false }: { slug?: string; domain?: boolean; clientApp?: boolean }) {
  ...
  const paymentReturn = useMemo(() => {
    ...
    return result && orderId && ['success', 'error', 'return'].includes(result)
      ? { result: result as 'success' | 'error' | 'return', orderId, token: token || undefined }
      : null;
  }, [search]);
  ...
  const digitalOnlyCart = cart.length > 0 && cart.every(line => line.product.productType === 'DIGITAL');
  ...
  useEffect(() => {
    ...
    const loadPaymentStatus = async () => {
      ...
      for (let attempt = 0; attempt < 6; attempt += 1) {
        try {
          const status = domain
           ? await publicEcommerceApi.domainPaymentStatus(paymentReturn.orderId, paymentReturn.token)
             : await publicEcommerceApi.paymentStatus(slug ?? '', paymentReturn.orderId, paymentReturn.token);
          if (cancelled) return;
          setSubmitted({
            ...
            failureReason: status.failureReason,
             digitalDownloads: status.digitalDownloads,
          });
    ...
    try {
      ...
      const returnUrl = () => {
        ...
        url.searchParams.set('order', order.id);
         if (order.downloadToken) url.searchParams.set('token', order.downloadToken);
        return url.toString();
      };
     ...
     <header className="relative border-b border-black/5 bg-white/95 text-[hsl(var(--foreground))] shadow-[0_1px_0_rgba(15,23,42,.03)] backdrop-blur">
      ...
      {!isStandalonePwa() && (installAvailable || isIosDevice()) && <div className="mb-6 flex flex-col gap-4 rounded-2xl border border-[var(--shop-primary)]/25 bg-[var(--shop-primary)]/10 p-4 sm:flex-row sm:items-center sm:justify-between">
       ...
       {submitted ? <PaymentResultPanel summary={submitted} currency={store.currency} onContinue={() => { setSubmitted(null); go(''); }} onOrders={customer ? () => { setSubmitted(null); go('/compte/commandes'); } : undefined} />
         ...
         : isCartRoute ? <CartPanel cart={cart} total={total} digitalOnly={digitalOnlyCart} store={store} customer={customer} form={checkoutForm} setForm={setCheckoutForm} paymentProvider={paymentProvider} setPaymentProvider={setPaymentProvider} onChange={change} onSubmit={() => void submitOrder()} submitting={submittingOrder} onBack={() => go('')} />
...
function PaymentResultPanel({ summary, currency, onContinue, onOrders }: { summary: PaymentSummary; currency: PublicShopBootstrap['store']['currency']; onContinue: () => void; onOrders?: () => void }) {
  ...
  return <section className="mx-auto max-w-xl rounded-3xl border bg-[hsl(var(--card))] p-8 text-center shadow-sm sm:p-10">
    ...
    <p className="mt-2 text-lg font-bold" style={{ color: 'var(--shop-primary)' }}>{money(summary.total, currency)}</p>
     {paid && summary.digitalDownloads.length > 0 && <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-left"><p className="text-sm font-bold text-emerald-900">Votre fichier est disponible</p><p className="mt-1 text-xs leading-5 text-emerald-800">Le téléchargement est débloqué immédiatement après la confirmation du paiement.</p><div className="mt-3 grid gap-2">{summary.digitalDownloads.map(download => <a key={download.itemId} href={download.url} className="inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-3 py-2.5 text-xs font-bold text-white hover:bg-emerald-800"><Download size={14} />Télécharger {download.fileName}</a>)}</div></div>}
...
function ProductDetail({ product, store, onBack, onAdd }: { product: PublicProduct; store: PublicShopBootstrap['store']; onBack: () => void; onAdd: () => void }) {
  ...
  return <section className="mx-auto max-w-4xl"><button type="button" onClick={onBack} className="inline-flex items-center gap-2 text-sm font-semibold text-[hsl(var(--muted-foreground))]"><ArrowLeft size={15} />Retour à la boutique</button><div className="mt-6 grid gap-6 rounded-3xl border bg-[hsl(var(--card))] p-5 shadow-sm sm:grid-cols-2 sm:p-8"><div className="flex aspect-square items-center justify-center overflow-hidden rounded-2xl bg-[hsl(var(--muted)/.5)]">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} className="h-full w-full object-contain p-5" /> : <Package size={54} className="text-[hsl(var(--muted-foreground))]" />}</div><div className="flex flex-col justify-center"><p className="text-xs font-bold uppercase tracking-[.16em]" style={{ color: 'var(--shop-primary)' }}>{isRental ? `LOCATION · ${rentalUnit}` : isDigital ? 'PRODUIT NUMÉRIQUE' : 'VENTE'} · {product.category}</p><h1 className="mt-3 text-3xl font-bold tracking-[-.04em]">{product.name}</h1><p className="mt-4 text-2xl font-bold" style={{ color: 'var(--shop-accent)' }}>{money(product.price, store.currency)}{isRental && <span className="ml-1 text-sm font-semibold">/ {rentalUnit}</span>}</p>{product.compareAtPrice && product.compareAtPrice > product.price && <p className="mt-1 text-sm text-[hsl(var(--muted-foreground))] line-through">{money(product.compareAtPrice, store.currency)}</p>}<p className="mt-5 whitespace-pre-line text-sm leading-7 text-[hsl(var(--muted-foreground))]">{product.description || 'Une référence sélectionnée par votre boutique.'}</p><p className={`mt-5 text-xs font-semibold ${isDigital ? 'text-emerald-700' : product.stock > 0 ? 'text-emerald-700' : 'text-amber-700'}`}>{isDigital ? 'Fichier disponible après paiement confirmé' : product.stock > 0 ? `${product.stock} unité${product.stock > 1 ? 's' : ''} disponible${product.stock > 1 ? 's' : ''}` : 'Indisponible'}</p><button type="button" onClick={onAdd} disabled={product.stock <= 0} className="mt-6 rounded-xl px-4 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50" style={{ backgroundColor: 'var(--shop-accent)' }}>{product.stock > 0 ? (isRental ? 'Ajouter une demande de location' : 'Ajouter au panier') : 'Indisponible'}</button></div></div></section>;
}
    ...
    {categories.map(category => {
      ...
      if (categoryProducts.length === 0) return null;
       return <section key={category}>
         <div className="mb-3 flex items-end justify-between gap-3 border-b border-black/5 pb-2"><div><p className="text-[9px] font-bold uppercase tracking-[.15em]" style={{ color: 'var(--shop-primary)' }}>Catégorie</p><h2 className="mt-1 text-lg font-bold tracking-[-.02em] sm:text-xl">{category}</h2></div><span className="text-[10px] font-semibold text-[hsl(var(--muted-foreground))]">{categoryProducts.length} produit{categoryProducts.length > 1 ? 's' : ''}</span></div>
           <div className="grid grid-cols-2 gap-2 min-[400px]:grid-cols-3 sm:grid-cols-4 sm:gap-3 lg:grid-cols-5 xl:grid-cols-6">
               {categoryProducts.map(product => <PublicOfferCard key={`product-${product.slug}`} imageUrl={product.imageUrl} icon={Package} badge={`${product.productType === 'DIGITAL' ? 'Produit numérique' : 'Produit'} · ${product.category}`} name={product.name} price={money(product.price, store.currency)} priceValue={product.price} compareAtPrice={product.compareAtPrice} availability={product.productType === 'DIGITAL' ? 'Téléchargement après paiement' : product.stock > 0 ? `${product.stock} en stock` : 'Indisponible'} store={store} onOpen={() => onProduct(product)} onAdd={() => onAdd(product)} />)}
        </div>
      </section>;
    })}
  </div>;
