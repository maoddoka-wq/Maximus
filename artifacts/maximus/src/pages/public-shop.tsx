...
function PublicImmobilierPage({ listings, store, slug, domain, form, setForm, submitted, onSubmitted }: {
  ...
  return <section className="space-y-6">
    ...
    {listings.length === 0 ? <div className="rounded-2xl border border-dashed p-12 text-center text-sm text-[hsl(var(--muted-foreground))]">Aucune annonce immobilière n’est publiée pour le moment.</div> : <div className="grid grid-cols-2 gap-2 min-[480px]:gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{listings.map(listing => <article key={listing.id} className="group flex min-w-0 flex-col overflow-hidden rounded-xl border border-black/5 bg-white shadow-[0_3px_12px_rgba(15,23,42,.06)] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_8px_20px_rgba(15,23,42,.1)]"><ImmobilierGallery media={listing.gallery} title={listing.title} /><div className="flex flex-1 flex-col p-2.5 sm:p-3"><div className="flex items-center justify-between gap-2"><span className="truncate text-[8px] font-bold uppercase tracking-[.1em] text-[var(--shop-primary)]">{listing.transactionType === 'SALE' ? 'À vendre' : 'À louer'}</span>{listing.featured && <span className="shrink-0 rounded bg-[var(--shop-accent)]/15 px-1 py-0.5 text-[8px] font-bold">À la une</span>}</div><h2 className="mt-1 line-clamp-2 min-h-8 text-xs font-semibold leading-4 sm:text-sm">{listing.title}</h2><p className="mt-1 truncate text-[10px] text-[hsl(var(--muted-foreground))]">{listing.neighborhood ? `${listing.neighborhood}, ` : ''}{listing.city}</p><p className="mt-2 text-sm font-bold leading-4 sm:text-base" style={{ color: 'var(--shop-accent)' }}>{money(listing.price, store.currency)}</p><div className="mt-2 flex flex-wrap gap-1 text-[9px] font-semibold text-[hsl(var(--muted-foreground))]">{listing.areaM2 && <span className="rounded bg-[hsl(var(--muted))] px-1.5 py-1">{listing.areaM2} m²</span>}{listing.bedrooms !== null && <span className="rounded bg-[hsl(var(--muted))] px-1.5 py-1">{listing.bedrooms} ch.</span>}{listing.bathrooms !== null && <span className="rounded bg-[hsl(var(--muted))] px-1.5 py-1">{listing.bathrooms} sdb.</span>}</div><p className="mt-2 line-clamp-2 text-[10px] leading-4 text-[hsl(var(--muted-foreground))]">{listing.description || 'Contactez l’agence pour recevoir les détails du bien.'}</p><button type="button" onClick={() => setForm({ ...form, listingId: listing.id })} className="mt-auto pt-3"><span className="flex w-full items-center justify-center rounded-lg px-2 py-2 text-[10px] font-bold text-[var(--shop-accent-foreground)] sm:text-xs" style={{ backgroundColor: 'var(--shop-accent)' }}>Faire une demande</span></button></div></article>)}</div>}
...
function ImmobilierGallery({ media, title }: { media: PublicShopBootstrap['immobilierListings'][number]['gallery']; title: string }) {
  ...
  return <div className="space-y-1 bg-[hsl(var(--muted)/.35)] p-1.5">
    <div className="relative flex aspect-[4/3] items-center justify-center overflow-hidden rounded-lg bg-[hsl(var(--muted)/.45)]">
      {active?.type === 'video'
        ? <video src={active.url} title={title} muted playsInline autoPlay loop className="h-full w-full object-cover" />
        : active?.url
          ? <img src={active.url} alt={title} className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.03]" />
          : <Building2 size={28} style={{ color: 'var(--shop-primary)' }} />}
      {media.some(item => item.type === 'video') && <span className="absolute bottom-1 left-1 rounded bg-black/60 px-1.5 py-0.5 text-[8px] font-bold text-white">Vidéo</span>}
      {media.length > 1 && <span className="absolute right-1 top-1 rounded bg-black/60 px-1.5 py-0.5 text-[8px] font-bold text-white">{selected + 1}/{media.length}</span>}
    </div>
    {media.length > 1 && <div className="flex gap-1 overflow-x-auto pb-0.5">{media.map((item, index) => <button type="button" key={item.id} onClick={() => setSelected(index)} className={`h-8 w-10 shrink-0 overflow-hidden rounded border ${index === selected ? 'border-[var(--shop-primary)]' : 'border-transparent'}`} aria-label={`Média ${index + 1}`} >{item.type === 'video' ? <video src={item.url} muted playsInline className="h-full w-full object-cover" /> : <img src={item.url} alt="" className="h-full w-full object-cover" />}</button>)}</div>}
  </div>;
}
