...
import { ArrowLeft, ArrowRight, Check, Clock3, Download, Grid2X2, Heart, Home, LockKeyhole, LogIn, Mail, MapPin, Menu, Minus, Package, Phone, Plus, RefreshCw, Search, ShoppingBag, Sparkles, Store, Truck, UserRound, X } from 'lucide-react';
     ...
     <header className="relative border-b border-black/5 bg-white/95 text-[hsl(var(--foreground))] shadow-[0_1px_0_rgba(15,23,42,.03)] backdrop-blur">
       <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-3.5 sm:px-6 lg:px-8">
          ...
          <div className="flex shrink-0 items-center gap-0.5 sm:hidden">
            <button type="button" onClick={() => go(customer ? '/compte' : '/connexion')} className="relative rounded-xl p-2.5 text-[hsl(var(--foreground))] transition hover:bg-[hsl(var(--muted))]" aria-label={customer ? 'Ouvrir mon compte' : 'Se connecter'}><UserRound size={19} /></button>
            <button type="button" onClick={() => go('/panier')} className="relative rounded-xl p-2.5 text-[hsl(var(--foreground))] transition hover:bg-[hsl(var(--muted))]" aria-label={`Ouvrir le panier${cartCount > 0 ? ` (${cartCount} article${cartCount > 1 ? 's' : ''})` : ''}`}><ShoppingBag size={20} />{cartCount > 0 && <span className="absolute right-0.5 top-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-[var(--shop-accent)] px-1 text-[9px] font-bold text-white">{cartCount}</span>}</button>
            <button type="button" className="rounded-xl p-2.5 text-[hsl(var(--foreground))] transition hover:bg-[hsl(var(--muted))]" onClick={() => setMobileMenu(open => !open)} aria-label="Ouvrir le menu" aria-expanded={mobileMenu} aria-controls="mobile-shop-menu"><Menu size={21} /></button>
          </div>
      ...
      {!isAuthRoute && !submitted && <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-black/5 bg-white/95 px-2 pb-[max(.5rem,env(safe-area-inset-bottom))] pt-2 shadow-[0_-8px_24px_rgba(15,23,42,.08)] backdrop-blur sm:hidden" aria-label="Navigation mobile"><div className="mx-auto grid max-w-md grid-cols-4 gap-1"><button type="button" onClick={() => go('')} className={`flex flex-col items-center gap-1 rounded-xl px-2 py-1.5 text-[10px] font-semibold ${isPublicNavActive('') ? 'text-[var(--shop-accent)]' : 'text-[hsl(var(--muted-foreground))]'}`}><Home size={18} />Accueil</button><button type="button" onClick={() => { if (routePath !== shopPath('')) go(''); window.setTimeout(() => document.getElementById('shop-catalog-search')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 0); }} className="flex flex-col items-center gap-1 rounded-xl px-2 py-1.5 text-[10px] font-semibold text-[hsl(var(--muted-foreground))]"><Grid2X2 size={18} />Catégories</button><button type="button" onClick={() => go('/panier')} className={`relative flex flex-col items-center gap-1 rounded-xl px-2 py-1.5 text-[10px] font-semibold ${isPublicNavActive('/panier') ? 'text-[var(--shop-accent)]' : 'text-[hsl(var(--muted-foreground))]'}`}><ShoppingBag size={18} />Panier{cartCount > 0 && <span className="absolute right-5 top-0 flex h-4 min-w-4 items-center justify-center rounded-full bg-[var(--shop-accent)] px-1 text-[9px] font-bold text-white">{cartCount}</span>}</button><button type="button" onClick={() => go(customer ? '/compte' : '/connexion')} className={`flex flex-col items-center gap-1 rounded-xl px-2 py-1.5 text-[10px] font-semibold ${customer && isAccountRoute ? 'text-[var(--shop-accent)]' : 'text-[hsl(var(--muted-foreground))]'}`}><UserRound size={18} />Compte</button></div></nav>}
