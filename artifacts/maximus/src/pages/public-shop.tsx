  ...
  return <div className="min-h-screen bg-[hsl(var(--background))]" style={{ '--shop-primary': store.primaryColor, '--shop-accent': store.accentColor } as React.CSSProperties}>
    ...
    <main className="mx-auto max-w-6xl px-5 py-8 sm:px-8 sm:py-10">
      ...
      {submitted ? <section className="mx-auto max-w-xl rounded-3xl border bg-[hsl(var(--card))] p-8 text-center shadow-sm sm:p-10"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-emerald-100 text-emerald-700"><Check size={26} /></span><h1 className="mt-5 text-2xl font-bold">Commande confirmée</h1><p className="mt-2 text-sm leading-6 text-[hsl(var(--muted-foreground))]">Votre commande est enregistrée sous la référence <strong className="text-[hsl(var(--foreground))]">{submitted.reference}</strong>.</p><p className="mt-4 text-lg font-bold" style={{ color: 'var(--shop-primary)' }}>{money(submitted.total, store.currency)}</p><button type="button" onClick={() => { setSubmitted(null); go(''); }} className="mt-7 rounded-xl px-5 py-3 text-sm font-bold text-white" style={{ backgroundColor: 'var(--shop-accent)' }}>Continuer mes achats</button></section>
        ...
        : <><section className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-xs font-bold uppercase tracking-[.18em]" style={{ color: 'var(--shop-primary)' }}>Sélection de la boutique</p><h1 className="mt-2 text-3xl font-bold tracking-[-.04em] sm:text-4xl">Trouvez ce qu’il vous faut.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[hsl(var(--muted-foreground))]">Commandez en ligne et retrouvez vos commandes dans votre espace client.</p></div><button type="button" onClick={() => go('/compte/favoris')} className="inline-flex items-center gap-2 self-start rounded-xl border px-3 py-2.5 text-sm font-bold sm:self-auto"><Heart size={16} />Favoris</button></section>{products.length === 0 ? <div className="rounded-2xl border border-dashed p-12 text-center text-sm text-[hsl(var(--muted-foreground))]">Aucun produit n’est disponible pour le moment.</div> : <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{products.map(product => <article key={product.slug} className="overflow-hidden rounded-2xl border bg-[hsl(var(--card))] shadow-sm"><div className="relative flex aspect-[4/3] items-center justify-center bg-[hsl(var(--muted)/.55)]">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} className="h-full w-full object-cover" /> : <Package size={38} className="text-[hsl(var(--muted-foreground))]" />}<button type="button" onClick={() => void toggleFavorite(product)} className="absolute right-3 top-3 rounded-full bg-[hsl(var(--card)/.9)] p-2 shadow-sm" aria-label={isFavorite(product) ? 'Retirer des favoris' : 'Ajouter aux favoris'}><Heart size={17} fill={isFavorite(product) ? 'currentColor' : 'none'} className={isFavorite(product) ? 'text-red-600' : ''} /></button></div><div className="p-4"><p className="text-[11px] font-bold uppercase tracking-[.14em] text-[hsl(var(--muted-foreground))]">{product.category}</p><h2 className="mt-2 text-base font-bold">{product.name}</h2><p className="mt-2 min-h-10 text-sm leading-5 text-[hsl(var(--muted-foreground))]">{product.description || 'Une référence sélectionnée par votre boutique.'}</p><div className="mt-5 flex items-center justify-between gap-3"><p className="text-lg font-bold" style={{ color: 'var(--shop-accent)' }}>{money(product.price, store.currency)}</p><button type="button" onClick={() => add(product)} className="rounded-xl px-3.5 py-2.5 text-xs font-bold text-white" style={{ backgroundColor: 'var(--shop-accent)' }}>Ajouter</button></div></div></article>)}</div>}</>}
    </main>
  </div>;
}
