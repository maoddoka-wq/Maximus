...
function DeliveryPage({ store, zones, customer, requests, form, setForm, submitted, onSubmit, submitting, taxiForm, setTaxiForm, taxiSubmitted, onTaxiSubmit, taxiSubmitting, onNavigate }: { store: PublicShopBootstrap['store']; zones: PublicShopBootstrap['deliveryZones']; customer: EcommerceCustomer | null; requests: EcommerceDeliveryRequest[]; form: { requesterName: string; requesterEmail: string; requesterPhone: string; address: string; deliveryZoneId: string; serviceType: EcommerceDeliveryServiceType; desiredDate: string; note: string }; setForm: (form: { requesterName: string; requesterEmail: string; requesterPhone: string; address: string; deliveryZoneId: string; serviceType: EcommerceDeliveryServiceType; desiredDate: string; note: string }) => void; submitted: EcommerceDeliveryRequest | null; onSubmit: () => void; submitting: boolean; taxiForm: { requesterName: string; requesterEmail: string; requesterPhone: string; pickupAddress: string; destinationAddress: string; passengerCount: number; note: string }; setTaxiForm: (form: { requesterName: string; requesterEmail: string; requesterPhone: string; pickupAddress: string; destinationAddress: string; passengerCount: number; note: string }) => void; taxiSubmitted: EcommerceTaxiRequest | null; onTaxiSubmit: () => void; taxiSubmitting: boolean; onNavigate: (path: string) => void }) {
  const steps = [
  ...
  return <section className="mx-auto max-w-5xl">
    ...
    <div className="mt-6 grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
     ...
     <div className="mt-6 rounded-2xl border bg-[hsl(var(--card))] p-5 shadow-sm sm:p-6">
