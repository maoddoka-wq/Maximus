...
function RentalPage({ products, store, onAdd, onBack }: { products: PublicProduct[]; store: PublicShopBootstrap['store']; onAdd: (product: PublicProduct) => void; onBack: () => void }) {
  const offers = products.filter(product => product.productType === 'RENTAL');
  ...
  return <section className="mx-auto max-w-5xl">
    ...
    <div className="mt-5 grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
      <div className="rounded-3xl p-6 text-white shadow-lg sm:p-8" style={{ background: `linear-gradient(145deg, ${store.accentColor}, ${store.primaryColor})` }}><span className="relative mono text-[10px] font-bold uppercase tracking-[.2em] text-white/70">Location & réservation</span><h1 className="mt-4 text-3xl font-bold tracking-[-.05em]">Des solutions prêtes pour vos projets.</h1><p className="mt-4 text-sm leading-7 text-white/75">Maison, bâche, voiture ou équipement : découvrez les offres publiées comme locations, sans les confondre avec les articles à vendre.</p><div className="mt-8 grid grid-cols-3 gap-2">{[['Maison', Home], ['Bâche', Sparkles], ['Voiture', Truck]].map(([label, Icon]) => <div key={label as string} className="rounded-xl bg-white/10 p-3 text-center"><Icon size={18} className="mx-auto" /><span className="mt-2 block text-[11px] font-bold">{label as string}</span></div>)}</div></div>
      <div className="rounded-3xl border bg-[hsl(var(--card))] p-5 shadow-sm sm:p-7"><div className="flex items-start justify-between gap-3"><div><span className="mono text-[10px] font-bold uppercase tracking-[.18em]" style={{ color: store.primaryColor }}>Offres disponibles</span><h2 className="mt-2 text-2xl font-bold">Trouvez votre prochaine location</h2></div><Home size={22} style={{ color: store.accentColor }} /></div>{offers.length === 0 ? <div className="mt-8 rounded-2xl border border-dashed p-8 text-center"><p className="text-sm font-semibold">Les offres de location arrivent bientôt.</p><p className="mt-2 text-xs leading-5 text-[hsl(var(--muted-foreground))]">Aucun produit n’est encore marqué comme Location dans le catalogue de cette boutique.</p></div> : <div className="mt-6 grid gap-3 sm:grid-cols-2">{offers.map(product => <article key={product.slug} className="overflow-hidden rounded-2xl border bg-white"><div className="flex aspect-[16/9] items-center justify-center bg-[hsl(var(--muted)/.5)]">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} className="h-full w-full object-cover" /> : <Home size={28} className="text-[hsl(var(--muted-foreground))]" />}</div><div className="p-4"><p className="text-[10px] font-bold uppercase tracking-[.14em]" style={{ color: store.primaryColor }}>LOCATION · {product.rentalPeriod ?? 'JOUR'}</p><h3 className="mt-2 font-bold">{product.name}</h3><p className="mt-1 text-sm font-bold" style={{ color: store.accentColor }}>{money(product.price, store.currency)} <span className="text-xs font-medium text-[hsl(var(--muted-foreground))]">/ {product.rentalPeriod === 'MOIS' ? 'mois' : product.rentalPeriod === 'SEMAINE' ? 'semaine' : 'jour'}</span></p><button type="button" onClick={() => onAdd(product)} className="mt-4 w-full rounded-xl py-2.5 text-xs font-bold text-white" style={{ backgroundColor: store.accentColor }}>Ajouter une demande</button></div></article>)}</div>}</div>
    </div>
  </section>;
}
