...
export default function PublicShopPage({ slug, domain = false }: { slug?: string; domain?: boolean }) {
  ...
  const categories = [...new Set([...products, ...rentals].map(item => item.category).filter(Boolean))].sort((a, b) => a.localeCompare(b, 'fr'));
  ...
  const visibleRentals = rentals.filter(rental => categoryFilter === 'ALL' || rental.category === categoryFilter).filter(rental => {
    const needle = searchQuery.trim().toLocaleLowerCase('fr-FR');
    return !needle || `${rental.name} ${rental.description} ${rental.category}`.toLocaleLowerCase('fr-FR').includes(needle);
  });
   const publicNav = [
    { label: 'Boutique', path: '' },
