  ...
  return <div className="min-h-screen w-full min-w-0 overflow-x-hidden bg-[hsl(var(--background))]" style={{ '--shop-primary': store.primaryColor, '--shop-accent': store.accentColor } as React.CSSProperties}>
    <header className="relative border-b bg-[var(--shop-accent)] text-white">
      <div className="flex w-full items-center justify-between gap-4 px-4 py-4 sm:px-6">
         <button type="button" onClick={() => go('')} className="flex min-w-0 max-w-[calc(100%-3rem)] shrink items-center gap-3 text-left"><span className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-[var(--shop-primary)] text-[var(--shop-accent)]">{store.logoUrl ? <img src={store.logoUrl} alt="" className="h-full w-full object-contain p-1" /> : <ShoppingBag size={21} />}</span><span className="min-w-0"><span className="block truncate text-lg font-bold">{store.name}</span><span className="block truncate text-xs text-white/65">{store.description}</span></span></button>
      ...
      {notice && <div className="mb-6 flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800"><Check size={16} /><span>{notice}</span><button type="button" className="ml-auto" onClick={() => setNotice('')} aria-label="Fermer"><X size={16} /></button></div>}
       {submitted ? <PaymentResultPanel summary={submitted} currency={store.currency} onContinue={() => { setSubmitted(null); go(''); }} onOrders={customer ? () => { setSubmitted(null); go('/compte/commandes'); } : undefined} />
        ...
        : isCartRoute ? <CartPanel cart={cart} total={total} store={store} customer={customer} form={checkoutForm} setForm={setCheckoutForm} onChange={change} onSubmit={() => void submitOrder()} onBack={() => go('')} />
         ...
         : isDeliveryRoute ? enabledFeatures.livraisons ? <DeliveryPage store={store} customer={customer} onNavigate={go} /> : <FeatureUnavailable title="Livraison non activée" text="Cette entreprise n’a pas encore autorisé la fonctionnalité livraison." onBack={() => go('')} />
         : isLocationRoute ? enabledFeatures.location ? <RentalPage products={products} store={store} onAdd={add} onBack={() => go('')} /> : <FeatureUnavailable title="Location non activée" text="Cette entreprise n’a pas encore autorisé la fonctionnalité location." onBack={() => go('')} />
       : productDetailSlug ? selectedProduct ? <ProductDetail product={selectedProduct} store={store} onBack={() => go('')} onAdd={() => add(selectedProduct)} /> : <div className="rounded-2xl border border-dashed p-12 text-center text-sm text-[hsl(var(--muted-foreground))]">Ce produit n’est plus disponible.</div>
        : <><section className="mb-8 flex min-w-0 flex-col justify-between gap-5 sm:flex-row sm:items-end"><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[.18em]" style={{ color: 'var(--shop-primary)' }}>Sélection de la boutique</p><h1 className="mt-2 text-3xl font-bold tracking-[-.04em] sm:text-4xl">Trouvez ce qu’il vous faut.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[hsl(var(--muted-foreground))]">Commandez en ligne et retrouvez vos commandes dans votre espace client.</p></div><button type="button" onClick={() => go('/compte/favoris')} className="inline-flex items-center gap-2 self-start rounded-xl border px-3 py-2.5 text-sm font-bold sm:self-auto"><Heart size={16} />Favoris</button></section><div className="mb-7 grid gap-3 sm:grid-cols-[1fr_auto_auto]"><label className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[hsl(var(--muted-foreground))]" size={16} /><input value={searchQuery} onChange={event => setSearchQuery(event.target.value)} placeholder="Rechercher dans la boutique" className="w-full rounded-xl border bg-[hsl(var(--card))] py-3 pl-10 pr-3 text-sm" /></label><select value={categoryFilter} onChange={event => setCategoryFilter(event.target.value)} className="rounded-xl border bg-[hsl(var(--card))] px-3 py-3 text-sm"><option value="ALL">Toutes les catégories</option>{categories.map(category => <option key={category} value={category}>{category}</option>)}</select><button type="button" onClick={() => go('/compte/favoris')} className="inline-flex items-center justify-center gap-2 rounded-xl border bg-[hsl(var(--card))] px-4 py-3 text-sm font-bold"><Heart size={16} fill={customer ? 'currentColor' : 'none'} className={customer ? 'text-red-600' : ''} />{customerData?.favoriteProductSlugs.length ?? 0}</button></div>{products.length === 0 ? <div className="rounded-2xl border border-dashed p-12 text-center text-sm text-[hsl(var(--muted-foreground))]">Aucun produit n’est disponible pour le moment.</div> : visibleProducts.length === 0 ? <div className="rounded-2xl border border-dashed p-12 text-center text-sm text-[hsl(var(--muted-foreground))]">Aucun produit ne correspond à votre recherche.</div> : <div className="grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-3">{visibleProducts.map(product => <article key={product.slug} className="min-w-0 overflow-hidden rounded-2xl border border-[#e8e0d4] bg-white shadow-sm"><button type="button" onClick={() => go(`/produit/${encodeURIComponent(product.slug)}`)} className="relative flex aspect-square w-full items-center justify-center bg-[#fbfaf7] sm:aspect-[16/9]">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} className="h-full w-full object-contain p-2 sm:p-5" /> : <Package size={30} className="text-[hsl(var(--muted-foreground))] sm:h-[38px] sm:w-[38px]" />}</button><div className="border-t border-[#e8e0d4] bg-[#f7f2ea] p-2.5 sm:p-4"><p className="truncate text-[9px] font-bold uppercase tracking-[.12em] text-[#8c6c37] sm:text-[11px] sm:tracking-[.14em]">{product.category}</p><div className="mt-1 flex min-w-0 items-start justify-between gap-1.5 sm:mt-2 sm:gap-3"><button type="button" onClick={() => go(`/produit/${encodeURIComponent(product.slug)}`)} className="min-w-0 break-words text-left text-sm font-bold leading-tight text-[#20252f] sm:text-base">{product.name}</button><p className="shrink-0 text-xs font-bold sm:text-base" style={{ color: 'var(--shop-accent)' }}>{money(product.price, store.currency)}</p></div><p className="mt-1.5 line-clamp-2 min-h-10 text-xs leading-4 text-[#655e55] sm:mt-2 sm:text-sm sm:leading-5">{product.description || 'Une référence sélectionnée par votre boutique.'}</p><div className="mt-3 flex items-center justify-between gap-2 sm:mt-5"><button type="button" onClick={() => void toggleFavorite(product)} className="rounded-lg border p-2" aria-label={isFavorite(product) ? 'Retirer des favoris' : 'Ajouter aux favoris'}><Heart size={15} fill={isFavorite(product) ? 'currentColor' : 'none'} className={isFavorite(product) ? 'text-red-600' : ''} /></button><button type="button" onClick={() => add(product)} className="flex-1 rounded-lg px-2 py-2 text-[11px] font-bold text-white sm:flex-none sm:px-3.5 sm:py-2.5 sm:text-xs" style={{ backgroundColor: 'var(--shop-accent)' }}>Ajouter</button></div></div></article>)}</div>}</>}
    </main>
...
function ProductDetail({ product, store, onBack, onAdd }: { product: PublicProduct; store: PublicShopBootstrap['store']; onBack: () => void; onAdd: () => void }) {
  return <section className="mx-auto max-w-4xl"><button type="button" onClick={onBack} className="inline-flex items-center gap-2 text-sm font-semibold text-[hsl(var(--muted-foreground))]"><ArrowLeft size={15} />Retour à la boutique</button><div className="mt-6 grid gap-6 rounded-3xl border bg-[hsl(var(--card))] p-5 shadow-sm sm:grid-cols-2 sm:p-8"><div className="flex aspect-square items-center justify-center overflow-hidden rounded-2xl bg-[hsl(var(--muted)/.5)]">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} className="h-full w-full object-contain p-5" /> : <Package size={54} className="text-[hsl(var(--muted-foreground))]" />}</div><div className="flex flex-col justify-center"><p className="text-xs font-bold uppercase tracking-[.16em]" style={{ color: 'var(--shop-primary)' }}>{product.category}</p><h1 className="mt-3 text-3xl font-bold tracking-[-.04em]">{product.name}</h1><p className="mt-4 text-2xl font-bold" style={{ color: 'var(--shop-accent)' }}>{money(product.price, store.currency)}</p>{product.compareAtPrice && product.compareAtPrice > product.price && <p className="mt-1 text-sm text-[hsl(var(--muted-foreground))] line-through">{money(product.compareAtPrice, store.currency)}</p>}<p className="mt-5 whitespace-pre-line text-sm leading-7 text-[hsl(var(--muted-foreground))]">{product.description || 'Une référence sélectionnée par votre boutique.'}</p><p className="mt-5 text-xs font-semibold text-emerald-700">{product.stock} unité{product.stock > 1 ? 's' : ''} disponible{product.stock > 1 ? 's' : ''}</p><button type="button" onClick={onAdd} className="mt-6 rounded-xl px-4 py-3 text-sm font-bold text-white" style={{ backgroundColor: 'var(--shop-accent)' }}>Ajouter au panier</button></div></div></section>;
}

function DeliveryPage({ store, customer, onNavigate }: { store: PublicShopBootstrap['store']; customer: EcommerceCustomer | null; onNavigate: (path: string) => void }) {
  const steps = [
  ...
  return <section className="mx-auto max-w-5xl">
    ...
    <div className="mt-6 flex items-center gap-3 rounded-2xl border border-[hsl(var(--primary)/.22)] bg-[hsl(var(--primary)/.08)] p-4 text-sm"><Clock3 size={18} className="shrink-0" style={{ color: store.accentColor }} /><span>Les délais et frais peuvent dépendre de votre zone. L’adresse enregistrée dans votre compte facilite chaque nouvelle commande.</span></div>
  </section>;
}

function RentalPage({ products, store, onAdd, onBack }: { products: PublicProduct[]; store: PublicShopBootstrap['store']; onAdd: (product: PublicProduct) => void; onBack: () => void }) {
  const keywords = ['location', 'maison', 'bâche', 'vehicule', 'véhicule', 'voiture', 'auto', 'utilitaire'];
  const offers = products.filter(product => keywords.some(keyword => `${product.category} ${product.name}`.toLocaleLowerCase('fr-FR').includes(keyword)));
  ...
  return <section className="mx-auto max-w-5xl">
    ...
    <div className="mt-5 grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
      <div className="rounded-3xl p-6 text-white shadow-lg sm:p-8" style={{ background: `linear-gradient(145deg, ${store.accentColor}, ${store.primaryColor})` }}><span className="relative mono text-[10px] font-bold uppercase tracking-[.2em] text-white/70">Location & réservation</span><h1 className="mt-4 text-3xl font-bold tracking-[-.05em]">Des solutions prêtes pour vos projets.</h1><p className="mt-4 text-sm leading-7 text-white/75">Maison, bâche, voiture ou équipement : découvrez les offres disponibles dans cette boutique.</p><div className="mt-8 grid grid-cols-3 gap-2">{[['Maison', Home], ['Bâche', Sparkles], ['Voiture', Truck]].map(([label, Icon]) => <div key={label as string} className="rounded-xl bg-white/10 p-3 text-center"><Icon size={18} className="mx-auto" /><span className="mt-2 block text-[11px] font-bold">{label as string}</span></div>)}</div></div>
      <div className="rounded-3xl border bg-[hsl(var(--card))] p-5 shadow-sm sm:p-7"><div className="flex items-start justify-between gap-3"><div><span className="mono text-[10px] font-bold uppercase tracking-[.18em]" style={{ color: store.primaryColor }}>Offres disponibles</span><h2 className="mt-2 text-2xl font-bold">Trouvez votre prochaine location</h2></div><Home size={22} style={{ color: store.accentColor }} /></div>{offers.length === 0 ? <div className="mt-8 rounded-2xl border border-dashed p-8 text-center"><p className="text-sm font-semibold">Les offres de location arrivent bientôt.</p><p className="mt-2 text-xs leading-5 text-[hsl(var(--muted-foreground))]">La boutique n’a pas encore publié de produit dans les catégories Maison, Bâche, Voiture ou Location.</p></div> : <div className="mt-6 grid gap-3 sm:grid-cols-2">{offers.map(product => <article key={product.slug} className="overflow-hidden rounded-2xl border bg-white"><div className="flex aspect-[16/9] items-center justify-center bg-[hsl(var(--muted)/.5)]">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} className="h-full w-full object-cover" /> : <Home size={28} className="text-[hsl(var(--muted-foreground))]" />}</div><div className="p-4"><p className="text-[10px] font-bold uppercase tracking-[.14em]" style={{ color: store.primaryColor }}>{product.category}</p><h3 className="mt-2 font-bold">{product.name}</h3><p className="mt-1 text-sm font-bold" style={{ color: store.accentColor }}>{money(product.price, store.currency)} <span className="text-xs font-medium text-[hsl(var(--muted-foreground))]">/ période</span></p><button type="button" onClick={() => onAdd(product)} className="mt-4 w-full rounded-xl py-2.5 text-xs font-bold text-white" style={{ backgroundColor: store.accentColor }}>Ajouter une demande</button></div></article>)}</div>}</div>
    </div>
  </section>;
}
...
function CustomerDashboard({ customer, orders, addresses, favoriteCount, store, onNavigate }: { customer: EcommerceCustomer; orders: EcommerceCustomerBootstrap['orders']; addresses: EcommerceCustomerAddress[]; favoriteCount: number; store: PublicShopBootstrap['store']; onNavigate: (path: string) => void }) {
  ...
  return <div className="space-y-5 fade-up">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <DashboardStat icon={Truck} label="En livraison" value={activeOrders.length} detail="À suivre" onClick={() => onNavigate('/compte/commandes')} />
