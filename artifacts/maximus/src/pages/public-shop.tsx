...
export default function PublicShopPage({ slug, domain = false }: { slug?: string; domain?: boolean }) {
  ...
  const add = (product: PublicProduct) => setCart(current => {
    ...
    if (existing) return current.map(line => line.product.slug === product.slug ? { ...line, quantity: Math.min(product.stock, line.quantity + 1) } : line);
    return [...current, { product, quantity: 1 }];
  });
  const change = (productSlug: string, delta: number) => setCart(current => current.flatMap(line => {
    ...
    try {
       const order = domain
         ? await publicEcommerceApi.createDomainOrder({
             ...form,
              items: cart.map(line => ({ productSlug: line.product.slug, quantity: line.quantity })),
           })
  ...
  return <div className="min-h-screen bg-[hsl(var(--background))]" style={{ '--shop-primary': store.primaryColor, '--shop-accent': store.accentColor } as React.CSSProperties}>
    ...
    <main className="mx-auto max-w-6xl px-5 py-10 sm:px-8">
      ...
      {submitted ? <section className="mx-auto max-w-xl rounded-3xl border bg-[hsl(var(--card))] p-10 text-center shadow-sm"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-emerald-100 text-emerald-700"><Check size={26} /></span><h1 className="mt-5 text-2xl font-bold">Commande confirmée</h1><p className="mt-2 text-sm leading-6 text-[hsl(var(--muted-foreground))]">Merci pour votre commande. Votre référence est <strong className="text-[hsl(var(--foreground))]">{submitted.reference}</strong>.</p><p className="mt-4 text-lg font-bold" style={{ color: 'var(--shop-primary)' }}>{money(submitted.total, store.currency)}</p><button type="button" onClick={() => setSubmitted(null)} className="mt-7 rounded-xl px-5 py-3 text-sm font-bold text-white" style={{ backgroundColor: 'var(--shop-accent)' }}>Continuer mes achats</button></section> : <>
        ...
        {products.length === 0 ? (
          <div className="rounded-2xl border border-dashed p-12 text-center text-sm text-[hsl(var(--muted-foreground))]">
            La boutique prépare actuellement son catalogue.
          </div>
        ) : (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {products.map((product) => (
              <article key={product.slug} className="overflow-hidden rounded-2xl border bg-[hsl(var(--card))] shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
                <div className="flex aspect-[4/3] items-center justify-center bg-[hsl(var(--muted)/.35)]">
                  {product.imageUrl ? <img src={product.imageUrl} alt="" className="h-full w-full object-cover" /> : <ShoppingBag size={40} className="text-[hsl(var(--primary)/.55)]" />}
                </div>
                <div className="p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-[.14em] text-[hsl(var(--muted-foreground))]">{product.category}</p>
                      <h2 className="mt-1 font-bold">{product.name}</h2>
                    </div>
                    {product.featured && <span className="rounded-full bg-[var(--shop-primary)]/15 px-2 py-1 text-[10px] font-bold" style={{ color: 'var(--shop-accent)' }}>À la une</span>}
                  </div>
                  <p className="mt-3 min-h-10 text-sm leading-5 text-[hsl(var(--muted-foreground))]">{product.description || 'Une référence sélectionnée par votre boutique.'}</p>
                  <div className="mt-5 flex items-center justify-between gap-3">
                    <div>
                      <p className="text-lg font-bold" style={{ color: 'var(--shop-accent)' }}>{money(product.price, store.currency)}</p>
                      {product.compareAtPrice && product.compareAtPrice > product.price && <p className="text-xs text-[hsl(var(--muted-foreground))] line-through">{money(product.compareAtPrice, store.currency)}</p>}
                    </div>
                    <button type="button" onClick={() => add(product)} className="rounded-xl px-3.5 py-2.5 text-xs font-bold text-white" style={{ backgroundColor: 'var(--shop-accent)' }}>Ajouter</button>
                  </div>
                </div>
              </article>
            ))}
