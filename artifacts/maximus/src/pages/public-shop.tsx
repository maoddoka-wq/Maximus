       ...
       {formOpen && !trip && !restoringTrip && !restoreError && <form id="transport-booking-form" onSubmit={submit} className="mt-5 space-y-3">
        <section className="border border-[#d9dcd6] bg-[#f8f9f5] p-4" aria-label="Détails du trajet">
          ...
          <div className="space-y-4 pt-4">
             <div className="flex items-start gap-3"><div className="flex h-8 w-8 shrink-0 items-center justify-center bg-[var(--transport-primary)] text-[var(--transport-accent)]"><MapPin size={15} /></div><div><p className="text-[10px] font-bold uppercase tracking-[.1em] text-[#7b8182]">Départ</p><p className="mt-1 text-[13px] font-bold">Position actuelle · Dakar</p>{locationState === 'error' && <button type="button" onClick={locate} className="mt-1 text-[11px] font-bold text-rose-700 underline">{locationMessage || 'Autoriser ma position'}</button>}</div></div>
