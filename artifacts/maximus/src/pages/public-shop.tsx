...
export default function PublicShopPage({ slug, domain = false }: { slug?: string; domain?: boolean }) {
  ...
  const [submitted, setSubmitted] = useState<Awaited<ReturnType<typeof publicEcommerceApi.createOrder>> | null>(null);
  const [checkoutKey, setCheckoutKey] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [categoryFilter, setCategoryFilter] = useState('ALL');
  const [authMode, setAuthMode] = useState<'login' | 'register'>('login');
  const [authForm, setAuthForm] = useState({ name: '', email: '', phone: '', password: '' });
  const [checkoutForm, setCheckoutForm] = useState({ customerName: '', customerEmail: '', customerPhone: '', paymentMethod: 'WAVE' as 'WAVE' | 'ORANGE_MONEY', shippingAddress: '', note: '' });
  ...
  return <div className="min-h-screen w-full min-w-0 overflow-x-hidden bg-[hsl(var(--background))]" style={{ '--shop-primary': store.primaryColor, '--shop-accent': store.accentColor } as React.CSSProperties}>
    ...
    <main className="mx-auto w-full min-w-0 max-w-6xl overflow-x-hidden px-4 py-8 sm:px-8 sm:py-10">
      ...
      {submitted ? <section className="mx-auto max-w-xl rounded-3xl border bg-[hsl(var(--card))] p-8 text-center shadow-sm sm:p-10"><span className={`mx-auto flex h-14 w-14 items-center justify-center rounded-full ${submittedFailed ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'}`}>{submittedFailed ? <X size={26} /> : <Check size={26} />}</span><h1 className="mt-5 text-2xl font-bold">{submittedPaid ? 'Commande confirmée' : submittedFailed ? 'Paiement non abouti' : 'Retour du paiement'}</h1><p className="mt-2 text-sm leading-6 text-[hsl(var(--muted-foreground))]">Votre commande est enregistrée sous la référence <strong className="text-[hsl(var(--foreground))]">{submitted.reference}</strong>.</p><p className="mt-4 text-lg font-bold" style={{ color: 'var(--shop-primary)' }}>{money(submitted.total, store.currency)}</p><div className="mt-5 rounded-2xl bg-[hsl(var(--muted)/.5)] p-4 text-left text-sm"><p className="font-bold">Paiement : {submitted.payment?.status ?? 'EN ATTENTE'}</p><p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">{submitted.payment?.providerMessage ?? (submittedPaid ? 'Le paiement a été confirmé.' : submittedFailed ? 'Le paiement n’a pas été confirmé.' : 'Le statut du paiement est en cours de vérification.')}</p>{submitted.payment?.checkoutUrl && !submittedPaid && !submittedFailed && <a href={submitted.payment.checkoutUrl} className="mt-3 inline-flex rounded-xl px-4 py-2 font-bold text-white" style={{ backgroundColor: 'var(--shop-accent)' }}>Reprendre le paiement</a>}</div><button type="button" onClick={() => { setSubmitted(null); go(''); }} className="mt-7 rounded-xl px-5 py-3 text-sm font-bold text-white" style={{ backgroundColor: 'var(--shop-accent)' }}>Continuer mes achats</button></section>
       ...
       : <><section className="mb-8 flex min-w-0 flex-col justify-between gap-5 sm:flex-row sm:items-end"><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[.18em]" style={{ color: 'var(--shop-primary)' }}>Sélection de la boutique</p><h1 className="mt-2 text-3xl font-bold tracking-[-.04em] sm:text-4xl">Trouvez ce qu’il vous faut.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[hsl(var(--muted-foreground))]">Commandez en ligne et retrouvez vos commandes dans votre espace client.</p></div><button type="button" onClick={() => go('/compte/favoris')} className="inline-flex items-center gap-2 self-start rounded-xl border px-3 py-2.5 text-sm font-bold sm:self-auto"><Heart size={16} />Favoris</button></section><div className="mb-7 grid gap-3 sm:grid-cols-[1fr_auto]"><label className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[hsl(var(--muted-foreground))]" size={16} /><input value={searchQuery} onChange={event => setSearchQuery(event.target.value)} placeholder="Rechercher dans la boutique" className="w-full rounded-xl border bg-[hsl(var(--card))] py-3 pl-10 pr-3 text-sm" /></label><select value={categoryFilter} onChange={event => setCategoryFilter(event.target.value)} className="rounded-xl border bg-[hsl(var(--card))] px-3 py-3 text-sm"><option value="ALL">Toutes les catégories</option>{categories.map(category => <option key={category} value={category}>{category}</option>)}</select></div>{products.length === 0 ? <div className="rounded-2xl border border-dashed p-12 text-center text-sm text-[hsl(var(--muted-foreground))]">Aucun produit n’est disponible pour le moment.</div> : visibleProducts.length === 0 ? <div className="rounded-2xl border border-dashed p-12 text-center text-sm text-[hsl(var(--muted-foreground))]">Aucun produit ne correspond à votre recherche.</div> : <div className="grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-3">{visibleProducts.map(product => <article key={product.slug} className="min-w-0 overflow-hidden rounded-2xl border border-[#e8e0d4] bg-white shadow-sm"><button type="button" onClick={() => go(`/produit/${encodeURIComponent(product.slug)}`)} className="relative flex aspect-square w-full items-center justify-center bg-[#fbfaf7] sm:aspect-[16/9]">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} className="h-full w-full object-contain p-2 sm:p-5" /> : <Package size={30} className="text-[hsl(var(--muted-foreground))] sm:h-[38px] sm:w-[38px]" />}</button><div className="border-t border-[#e8e0d4] bg-[#f7f2ea] p-2.5 sm:p-4"><p className="truncate text-[9px] font-bold uppercase tracking-[.12em] text-[#8c6c37] sm:text-[11px] sm:tracking-[.14em]">{product.category}</p><div className="mt-1 flex min-w-0 items-start justify-between gap-1.5 sm:mt-2 sm:gap-3"><button type="button" onClick={() => go(`/produit/${encodeURIComponent(product.slug)}`)} className="min-w-0 break-words text-left text-sm font-bold leading-tight text-[#20252f] sm:text-base">{product.name}</button><p className="shrink-0 text-xs font-bold sm:text-base" style={{ color: 'var(--shop-accent)' }}>{money(product.price, store.currency)}</p></div><p className="mt-1.5 line-clamp-2 min-h-10 text-xs leading-4 text-[#655e55] sm:mt-2 sm:text-sm sm:leading-5">{product.description || 'Une référence sélectionnée par votre boutique.'}</p><div className="mt-3 flex items-center justify-between gap-2 sm:mt-5"><button type="button" onClick={() => void toggleFavorite(product)} className="rounded-lg border p-2" aria-label={isFavorite(product) ? 'Retirer des favoris' : 'Ajouter aux favoris'}><Heart size={15} fill={isFavorite(product) ? 'currentColor' : 'none'} className={isFavorite(product) ? 'text-red-600' : ''} /></button><button type="button" disabled={product.stock <= 0} onClick={() => add(product)} className="flex-1 rounded-lg px-2 py-2 text-[11px] font-bold text-white disabled:cursor-not-allowed disabled:opacity-50 sm:flex-none sm:px-3.5 sm:py-2.5 sm:text-xs" style={{ backgroundColor: 'var(--shop-accent)' }}>{product.stock > 0 ? 'Ajouter' : 'Rupture de stock'}</button></div></div></article>)}</div>}</>}
    </main>
  </div>;
}
