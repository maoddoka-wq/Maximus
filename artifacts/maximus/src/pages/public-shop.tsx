      ...
      <div className="rounded-[1.25rem] border bg-white p-4 shadow-sm sm:rounded-[1.75rem] sm:p-7">
        ...
        {formOpen && !trip && <form onSubmit={submit} className="space-y-5">
          ...
          {quoteLoading && <p className="inline-flex items-center gap-2 rounded-xl bg-sky-50 px-4 py-3 text-xs font-semibold text-sky-800"><RefreshCw size={14} className="animate-spin" />Calcul de la route et du tarif…</p>}
           {quoteError && !quoteLoading && <p role="alert" className="rounded-xl bg-amber-50 px-4 py-3 text-xs text-amber-900">{quoteError} Vous pouvez tout de même demander un chauffeur ; le tarif final sera calculé ensuite.</p>}
