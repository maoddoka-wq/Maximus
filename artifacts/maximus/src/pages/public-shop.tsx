...
import { ArrowDownToLine, ArrowLeft, ArrowRight, BriefcaseBusiness, CarFront, Check, Clock3, Download, Heart, Home, LockKeyhole, LogIn, Mail, MapPin, MessageCircle, Minus, Package, Phone, Plane, Plus, RefreshCw, Search, ShoppingBag, Sparkles, Store, Truck, UserRound, X } from 'lucide-react';
...
function TransportPublicPage({ store, slug, domain, onBack }: { store: PublicShopBootstrap['store']; slug?: string; domain?: boolean; onBack: () => void }) {
  ...
  const quickDestinations = [
    { label: 'Plateau', icon: Home },
    { label: 'Almadies', icon: BriefcaseBusiness },
    { label: 'Aéroport de Dakar', icon: Plane },
  ];
  const handleFormSubmit = (event: FormEvent) => {
    if (!formOpen) {
      event.preventDefault();
      setFormOpen(true);
      if (locationState !== 'ready') locate();
      return;
    }
    ...
    <div className="relative z-30 -mt-1 min-h-0 flex-1 overflow-y-auto rounded-t-[2.25rem] bg-white px-4 pb-5 pt-5 shadow-[0_-12px_30px_rgba(15,23,42,.12)] sm:mx-auto sm:-mt-10 sm:max-w-3xl sm:rounded-[2rem] sm:p-7 sm:shadow-xl">
      <div className="mx-auto h-1.5 w-12 rounded-full bg-slate-200 sm:hidden" />
         ...
         {!formOpen && !trip && <div className="mt-4 sm:mt-0">
          <p className="text-xs font-bold uppercase tracking-[.16em] text-[var(--shop-accent)]">Transport</p>
          <h1 className="mt-2 text-[1.7rem] font-black leading-tight tracking-[-.05em] text-slate-950 sm:text-3xl">Où allez-vous ?</h1>
          <div className="mt-5 flex items-center gap-3 rounded-2xl bg-slate-100 px-4 py-4 focus-within:ring-2 focus-within:ring-[var(--shop-accent)]/20">
            <Search size={23} className="shrink-0 text-slate-500" />
            <label className="min-w-0 flex-1"><span className="sr-only">Destination</span><input value={form.destination} onChange={event => { setSelectedPlace(null); setForm(current => ({ ...current, destination: event.target.value })); }} className="w-full bg-transparent text-sm font-semibold text-slate-900 outline-none placeholder:text-slate-500" placeholder="Entrez votre destination" autoComplete="off" /></label>
          </div>
          <div className="mt-3 flex gap-2 overflow-x-auto pb-1">{quickDestinations.map(({ label, icon: Icon }) => <button type="button" key={label} onClick={() => { setSelectedPlace(null); setForm(current => ({ ...current, destination: label })); }} className="inline-flex shrink-0 items-center gap-2 rounded-full border border-slate-200 bg-white px-3.5 py-2.5 text-xs font-bold text-slate-700 transition hover:border-[var(--shop-accent)] hover:text-[var(--shop-accent)]"><Icon size={16} />{label}</button>)}</div>
          {locationMessage && locationState === 'error' && <p className="mt-3 rounded-xl bg-rose-50 px-3.5 py-3 text-xs text-rose-800">{locationMessage}</p>}
          <button type="button" onClick={() => { setFormOpen(true); if (locationState !== 'ready') locate(); }} disabled={locationState === 'locating'} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[var(--shop-accent)] px-5 py-4 text-sm font-black text-white shadow-lg shadow-black/10 transition hover:translate-y-[-1px] disabled:cursor-wait disabled:opacity-60">{locationState === 'locating' ? <RefreshCw size={17} className="animate-spin" /> : <CarFront size={18} />}{locationState === 'locating' ? 'Localisation en cours…' : 'Commander un taxi'}<ArrowRight size={16} /></button>
         </div>}
         {formOpen && !trip && <form onSubmit={handleFormSubmit} className="space-y-5">
       ...
       {!trip && (phone || whatsappHref) && <div className="mt-4 grid grid-cols-2 gap-3 border-t border-slate-100 pt-4"><p className="col-span-2 text-center text-xs font-semibold text-slate-500">Besoin d’aide ? Contactez {store.name}.</p>{phone && <a href={`tel:${phone}`} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 px-3 py-3 text-xs font-bold text-slate-800"><Phone size={16} />Appeler</a>}{whatsappHref && <a href={whatsappHref} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#25D366] px-3 py-3 text-xs font-bold text-white"><MessageCircle size={16} />WhatsApp</a>}</div>}
    </div>
  </section>;
}
