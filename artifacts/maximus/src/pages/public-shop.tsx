  ...
  return <div className="min-h-screen w-full min-w-0 overflow-x-hidden bg-[hsl(var(--background))]" style={{ '--shop-primary': store.primaryColor, '--shop-accent': store.accentColor } as React.CSSProperties}>
    ...
    <main className="mx-auto w-full min-w-0 max-w-6xl overflow-x-hidden px-4 py-8 sm:px-8 sm:py-10">
      ...
      {submitted ? <section className="mx-auto max-w-xl rounded-3xl border bg-[hsl(var(--card))] p-8 text-center shadow-sm sm:p-10"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-emerald-100 text-emerald-700"><Check size={26} /></span><h1 className="mt-5 text-2xl font-bold">Commande confirmée</h1><p className="mt-2 text-sm leading-6 text-[hsl(var(--muted-foreground))]">Votre commande est enregistrée sous la référence <strong className="text-[hsl(var(--foreground))]">{submitted.reference}</strong>.</p><p className="mt-4 text-lg font-bold" style={{ color: 'var(--shop-primary)' }}>{money(submitted.total, store.currency)}</p><button type="button" onClick={() => { setSubmitted(null); go(''); }} className="mt-7 rounded-xl px-5 py-3 text-sm font-bold text-white" style={{ backgroundColor: 'var(--shop-accent)' }}>Continuer mes achats</button></section>
       ...
       : <><section className="mb-8 flex min-w-0 flex-col justify-between gap-5 sm:flex-row sm:items-end"><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[.18em]" style={{ color: 'var(--shop-primary)' }}>Sélection de la boutique</p><h1 className="mt-2 text-3xl font-bold tracking-[-.04em] sm:text-4xl">Trouvez ce qu’il vous faut.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[hsl(var(--muted-foreground))]">Commandez en ligne et retrouvez vos commandes dans votre espace client.</p></div><button type="button" onClick={() => go('/compte/favoris')} className="inline-flex items-center gap-2 self-start rounded-xl border px-3 py-2.5 text-sm font-bold sm:self-auto"><Heart size={16} />Favoris</button></section>{products.length === 0 ? <div className="rounded-2xl border border-dashed p-12 text-center text-sm text-[hsl(var(--muted-foreground))]">Aucun produit n’est disponible pour le moment.</div> : <div className="grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-3">{products.map(product => <article key={product.slug} className="min-w-0 overflow-hidden rounded-2xl border border-[#e8e0d4] bg-white shadow-sm"><div className="relative flex aspect-square items-center justify-center bg-[#fbfaf7] sm:aspect-[16/9]">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} className="h-full w-full object-contain p-2 sm:p-5" /> : <Package size={30} className="text-[hsl(var(--muted-foreground))] sm:h-[38px] sm:w-[38px]" />}<button type="button" onClick={() => void toggleFavorite(product)} className="absolute right-2 top-2 rounded-full bg-white/90 p-1.5 shadow-sm sm:right-3 sm:top-3 sm:p-2" aria-label={isFavorite(product) ? 'Retirer des favoris' : 'Ajouter aux favoris'}><Heart size={15} fill={isFavorite(product) ? 'currentColor' : 'none'} className={isFavorite(product) ? 'text-red-600' : ''} /></button></div><div className="border-t border-[#e8e0d4] bg-[#f7f2ea] p-2.5 sm:p-4"><p className="truncate text-[9px] font-bold uppercase tracking-[.12em] text-[#8c6c37] sm:text-[11px] sm:tracking-[.14em]">{product.category}</p><div className="mt-1 flex min-w-0 items-start justify-between gap-1.5 sm:mt-2 sm:gap-3"><h2 className="min-w-0 break-words text-sm font-bold leading-tight text-[#20252f] sm:text-base">{product.name}</h2><p className="shrink-0 text-xs font-bold sm:text-base" style={{ color: 'var(--shop-accent)' }}>{money(product.price, store.currency)}</p></div><p className="mt-1.5 line-clamp-2 min-h-10 text-xs leading-4 text-[#655e55] sm:mt-2 sm:text-sm sm:leading-5">{product.description || 'Une référence sélectionnée par votre boutique.'}</p><div className="mt-3 flex justify-end sm:mt-5"><button type="button" onClick={() => add(product)} className="w-full rounded-lg px-2 py-2 text-[11px] font-bold text-white sm:w-auto sm:rounded-xl sm:px-3.5 sm:py-2.5 sm:text-xs" style={{ backgroundColor: 'var(--shop-accent)' }}>Ajouter</button></div></div></article>)}</div>}</>}
    </main>
  </div>;
}
