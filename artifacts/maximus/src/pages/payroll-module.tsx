...
const payrollFeatureIds = ['tableau-de-bord', 'bénéficiaires', 'préparer-une-paie', 'validation', 'virements', 'solde-de-paie', 'historique'] as const;

function Button({ children, onClick, primary = false, disabled = false, danger = false, loading = false }: { children: React.ReactNode; onClick?: () => void; primary?: boolean; disabled?: boolean; danger?: boolean; loading?: boolean }) {
  return <button type="button" disabled={disabled || loading} onClick={onClick} className={`app-action inline-flex items-center justify-center gap-2 rounded-lg px-3 py-2.5 text-xs font-bold transition disabled:cursor-not-allowed disabled:opacity-40 ${primary ? 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] hover:opacity-90' : danger ? 'border border-red-200 text-red-700 hover:bg-red-50' : 'border bg-[hsl(var(--card))] hover:bg-[hsl(var(--muted))]'}`}>{loading && <RefreshCw size={14} className="animate-spin" aria-hidden="true" />}{children}</button>;
}
...
export default function PayrollModulePage({ companyId: _companyId, employees = [], canCreate, canModify, visibleFeatureIds, activeFeatureId = 'tableau-de-bord', preview = false }: { companyId: string; employees?: { id: string; firstName: string; lastName: string }[]; canCreate: boolean; canModify: boolean; visibleFeatureIds?: string[]; activeFeatureId?: string; preview?: boolean }) {
  ...
  const visibleFeatures = useMemo(() => new Set(visibleFeatureIds ?? payrollFeatureIds), [visibleFeatureIds]);
  const activeFeature = normalizePayrollFeatureId(activeFeatureId) ?? 'tableau-de-bord';
  const showFeature = (featureId: string) => activeFeature === 'tableau-de-bord' || activeFeature === featureId;
  const canSee = (featureId: string) => visibleFeatures.has(featureId);
  ...
  const createBeneficiary = async () => {
    if (!beneficiaryForm.fullName || !beneficiaryForm.mobile || !beneficiaryForm.accountNumber || Number(beneficiaryForm.monthlySalary) <= 0) {
      showAppToast('Complétez le nom, le numéro, le compte et le salaire.', 'warning');
      return;
    }
  ...
  const createBatch = async () => {
    ...
    await mutate(() => api.createBatch({ period: batchPeriod, paymentDate, beneficiaryIds: selected }), 'Paie préparée.', 'batch');
    setSelected([]);
  };
  const topup = async () => {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.4fr_.8fr]">
        {canSee('bénéficiaires') && showFeature('bénéficiaires') && <Panel title="Bénéficiaires" action={canManageBeneficiaries && !preview ? <Button primary onClick={() => setShowBeneficiary(true)}><Plus size={15} />Ajouter</Button> : null}>
        {showBeneficiary && <div className="mb-5 rounded-xl border bg-[hsl(var(--muted)/.35)] p-4"><div className="mb-4 flex items-center justify-between"><strong>Nouveau bénéficiaire</strong><button type="button" onClick={() => setShowBeneficiary(false)}><X size={17} /></button></div><div className="grid gap-3 md:grid-cols-2"><label className="block text-xs font-bold">Employé existant<select value={beneficiaryForm.employeeId} onChange={event => { const employee = employees.find(item => item.id === event.target.value); setBeneficiaryForm({ ...beneficiaryForm, employeeId: event.target.value, fullName: employee ? `${employee.firstName} ${employee.lastName}` : beneficiaryForm.fullName }); }} className="mt-1.5 w-full rounded-lg border bg-[hsl(var(--card))] px-3 py-2.5 text-sm font-normal outline-none focus:border-[hsl(var(--primary))]"><option value="">Saisie libre</option>{employees.map(employee => <option key={employee.id} value={employee.id}>{employee.firstName} {employee.lastName}</option>)}</select></label><Field label="Nom complet" value={beneficiaryForm.fullName} onChange={value => setBeneficiaryForm({ ...beneficiaryForm, fullName: value })} /><Field label="Numéro Wave" value={beneficiaryForm.mobile} onChange={value => setBeneficiaryForm({ ...beneficiaryForm, mobile: value })} placeholder="+221…" /><Field label="Numéro de compte" value={beneficiaryForm.accountNumber} onChange={value => setBeneficiaryForm({ ...beneficiaryForm, accountNumber: value })} /><Field label="Salaire mensuel (FCFA)" value={beneficiaryForm.monthlySalary} onChange={value => setBeneficiaryForm({ ...beneficiaryForm, monthlySalary: value })} type="number" /><Field label="Jour de paiement" value={beneficiaryForm.paymentDay} onChange={value => setBeneficiaryForm({ ...beneficiaryForm, paymentDay: value })} type="number" /></div><div className="mt-4 flex justify-end"><Button primary onClick={() => void createBeneficiary()}>Enregistrer</Button></div></div>}
         {data.beneficiaries.length === 0 ? <div className="rounded-xl border border-dashed p-8 text-center text-sm text-[hsl(var(--muted-foreground))]">Aucun bénéficiaire enregistré.</div> : <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b text-xs text-[hsl(var(--muted-foreground))]"><th className="pb-3 pr-3">Bénéficiaire</th><th className="pb-3 pr-3">Compte</th><th className="pb-3 pr-3">Salaire</th><th className="pb-3 pr-3">Paiement</th><th className="pb-3"></th></tr></thead><tbody>{data.beneficiaries.map(item => <tr key={item.id} className="border-b last:border-0"><td className="py-3 pr-3"><div className="font-bold">{item.fullName}</div><div className="text-xs text-[hsl(var(--muted-foreground))]">{item.mobile}</div></td><td className="py-3 pr-3 font-mono text-xs">{item.accountNumberMasked}</td><td className="py-3 pr-3 font-semibold">{money(item.monthlySalary)}</td><td className="py-3 pr-3">Le {item.paymentDay}</td><td className="py-3 text-right">{canModifyBeneficiaries && !preview && <Button danger onClick={() => void mutate(() => api.archiveBeneficiary(item.id), 'Bénéficiaire archivé.')}><Trash2 size={14} /></Button>}</td></tr>)}</tbody></table></div>}
       </Panel>}
         {canSee('solde-de-paie') && showFeature('solde-de-paie') && <Panel title="Solde de paie" action={<Banknote size={18} className="text-[hsl(var(--primary))]" />}><p className="text-3xl font-bold">{money(data.wallet.availableBalance)}</p><p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">Réservé : {money(data.wallet.reservedBalance)}</p><div className="mt-5 flex gap-2"><input value={topupAmount} onChange={event => setTopupAmount(event.target.value)} type="number" min="1000" placeholder="Montant FCFA" className="min-w-0 flex-1 rounded-lg border bg-[hsl(var(--card))] px-3 py-2.5 text-sm outline-none" /><Button primary loading={pendingAction === 'topup'} disabled={!canManageBalance || preview} onClick={() => void topup()}><CreditCard size={15} />Recharger</Button></div><p className="mt-3 text-[11px] text-[hsl(var(--muted-foreground))]">Le solde est crédité uniquement après confirmation du checkout DiamanoPay.</p></Panel>}
    </div>
      {canSee('préparer-une-paie') && showFeature('préparer-une-paie') && <Panel title="Préparer une paie" action={<ShieldCheck size={18} className="text-[hsl(var(--primary))]" />}>
       <div className="grid gap-3 md:grid-cols-[1fr_1fr_1fr_auto]"><Field label="Période" value={batchPeriod} onChange={setBatchPeriod} type="month" /><Field label="Date de paiement" value={paymentDate} onChange={setPaymentDate} type="date" /><div className="flex items-end pb-1 text-sm font-bold">{selected.length} bénéficiaire(s) · {money(selectedTotal)}</div><div className="flex items-end"><Button primary disabled={!canPrepare || preview || !selected.length} onClick={() => void createBatch()}><Plus size={15} />Préparer</Button></div></div>
      ...
      {(canSee('historique') || activeFeature === 'validation' || activeFeature === 'virements') && (activeFeature === 'tableau-de-bord' || activeFeature === 'historique' || activeFeature === 'validation' || activeFeature === 'virements') && <Panel title={activeFeature === 'validation' ? 'Validation des paies' : activeFeature === 'virements' ? 'Virements de paie' : 'Historique des paies'} action={<History size={18} className="text-[hsl(var(--primary))]" />}>
        {data.batches.length === 0 ? <div className="rounded-xl border border-dashed p-8 text-center text-sm text-[hsl(var(--muted-foreground))]">Aucune paie préparée.</div> : <div className="space-y-3">{data.batches.map(batch => <BatchRow key={batch.id} batch={batch} canSubmit={(activeFeature === 'tableau-de-bord' || activeFeature === 'historique') && canPrepare && !preview} canValidate={(activeFeature === 'tableau-de-bord' || activeFeature === 'historique' || activeFeature === 'validation') && canValidate && !preview} canPayout={(activeFeature === 'tableau-de-bord' || activeFeature === 'historique' || activeFeature === 'virements') && canPayout && !preview} onSubmit={() => void mutate(() => api.submitBatch(batch.id), 'Paie envoyée pour validation.')} onApprove={() => void mutate(() => api.approveBatch(batch.id), 'Paie validée.')} onPayout={() => void mutate(() => api.payoutBatch(batch.id), 'Virements lancés.')} statusLabel={statusLabel} />)}</div>}
     </Panel>}
  </div>;
}

function BatchRow({ batch, canSubmit, canValidate, canPayout, onSubmit, onApprove, onPayout, statusLabel }: { batch: PayrollBatch; canSubmit: boolean; canValidate: boolean; canPayout: boolean; onSubmit: () => void; onApprove: () => void; onPayout: () => void; statusLabel: Record<string, string> }) {
  return <div className="flex flex-wrap items-center gap-4 rounded-xl border p-4"><div className="min-w-[150px] flex-1"><div className="font-bold">{batch.period}</div><div className="text-xs text-[hsl(var(--muted-foreground))]">{batch.itemCount} bénéficiaire(s) · paiement le {dateLabel(batch.paymentDate)}</div></div><div className="font-semibold">{money(batch.totalAmount)}</div><Badge>{statusLabel[batch.status] ?? batch.status}</Badge><div className="flex flex-wrap gap-2">{canSubmit && batch.status === 'DRAFT' && <Button onClick={onSubmit}>Soumettre</Button>}{canValidate && batch.status === 'PENDING_APPROVAL' && <Button primary onClick={onApprove}><Check size={14} />Valider</Button>}{canPayout && batch.status === 'APPROVED' && <Button primary onClick={onPayout}><CreditCard size={14} />Virer</Button>}</div></div>;
}