...
export default function PayrollModulePage({ companyId: _companyId, employees = [], canCreate, canModify, visibleFeatureIds, preview = false }: { companyId: string; employees?: { id: string; firstName: string; lastName: string }[]; canCreate: boolean; canModify: boolean; visibleFeatureIds?: string[]; preview?: boolean }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.4fr_.8fr]">
       {canSee('bénéficiaires') && <Panel title="Bénéficiaires" action={canManageBeneficiaries && !preview ? <Button primary onClick={() => setShowBeneficiary(true)}><Plus size={15} />Ajouter</Button> : null}>
     ...
     {canSee('historique') && <Panel title="Historique des paies" action={<History size={18} className="text-[hsl(var(--primary))]" />}>
