...
export default function PayrollModulePage({ companyId: _companyId, employees = [], canCreate, canModify, visibleFeatureIds, activeFeatureId = 'tableau-de-bord', preview = false }: { companyId: string; employees?: { id: string; firstName: string; lastName: string }[]; canCreate: boolean; canModify: boolean; visibleFeatureIds?: string[]; activeFeatureId?: string; preview?: boolean }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.4fr_.8fr]">
      ...
      {(canSee('historique') || activeFeature === 'validation' || activeFeature === 'virements') && showFeature('historique') && <Panel title={activeFeature === 'validation' ? 'Validation des paies' : activeFeature === 'virements' ? 'Virements de paie' : 'Historique des paies'} action={<History size={18} className="text-[hsl(var(--primary))]" />}>
       {data.batches.length === 0 ? <div className="rounded-xl border border-dashed p-8 text-center text-sm text-[hsl(var(--muted-foreground))]">Aucune paie préparée.</div> : <div className="space-y-3">{data.batches.map(batch => <BatchRow key={batch.id} batch={batch} canSubmit={canPrepare && !preview} canValidate={canValidate && !preview} canPayout={canPayout && !preview} onSubmit={() => void mutate(() => api.submitBatch(batch.id), 'Paie envoyée pour validation.')} onApprove={() => void mutate(() => api.approveBatch(batch.id), 'Paie validée.')} onPayout={() => void mutate(() => api.payoutBatch(batch.id), 'Virements lancés.')} statusLabel={statusLabel} />)}</div>}
     </Panel>}
  </div>;
}
