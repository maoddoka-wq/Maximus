...
type ReturnRecord = { id: string; reference: string; type: 'RETOUR CLIENT' | 'AVOIR FOURNISSEUR'; partner: string; amount: number; date: string; status: Status };
...
export default function CommerceModulePage({
  ...
  return <div className="space-y-5" data-testid="commerce-module">
    ...
    {tab === 'sales' && <SalesPage data={data} query={query} mutate={mutate} canCreate={canCreate} canModify={canModify} onToast={setToast} />}
    {tab === 'products' && <ProductsPage data={data} query={query} mutate={mutate} canCreate={canCreate} />}
    {tab === 'clients' && <ClientsPage state={state} query={query} canCreate={canCreate} onUpdate={updateState} />}
    {tab === 'suppliers' && <SuppliersPage data={data} query={query} canCreate={canCreate} mutate={mutate} />}
    {tab === 'purchases' && <PurchasesPage data={data} query={query} canCreate={canCreate} mutate={mutate} />}
    {tab === 'expenses' && <ExpensesPage state={state} query={query} canCreate={canCreate} onUpdate={updateState} />}
    {tab === 'cash' && <CashPage state={state} query={query} canCreate={canCreate} onUpdate={updateState} />}
    {tab === 'credit' && <CreditPage state={state} query={query} canModify={canModify} onUpdate={updateState} />}
    {tab === 'invoices' && <InvoicesPage data={data} query={query} onToast={setToast} />}
    {tab === 'returns' && <ReturnsPage state={state} query={query} canCreate={canCreate} canModify={canModify} onUpdate={updateState} />}
    ...
    {tab === 'team' && <TeamPage data={data} query={query} canModify={canModify} onToast={setToast} />}
...
function TeamPage({ data, query, canModify, onToast }: { data: StoreData; query: string; canModify: boolean; onToast: (message: string) => void }) {
...
function Field({ label, value, onChange, type = 'text', placeholder, help }: { label: string; value: string; onChange: (value: string) => void; type?: string; placeholder?: string; help?: string }) { return <label className="block text-xs font-bold">{label}<input type={type} value={value} placeholder={placeholder} onChange={event => onChange(event.target.value)} className="mt-2 w-full rounded-lg border bg-transparent px-3 py-2.5 text-sm font-normal outline-none focus:border-[hsl(var(--primary))]" /><span className="mt-1 block text-[10px] font-normal leading-4 text-[hsl(var(--muted-foreground))]">{help ?? `Saisissez ${label.toLowerCase()}.`}</span></label>; }
