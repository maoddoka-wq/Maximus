...
type EcommerceTab = 'dashboard' | 'accueil' | 'catalogue' | 'categories' | 'commandes' | 'clients' | 'promotions' | 'location' | 'livraisons' | 'transport' | 'finances' | 'parametres';
  ...
  return (
    <div className="space-y-5" data-testid="ecommerce-module" aria-busy={Boolean(pendingAction)}>
      ...
      {tab === 'promotions' && <Promotions />}
       ...
       {tab === 'transport' && <Transport data={data} canCreate={canCreate} canModify={canModify} run={run} />}
...
function Transport({ data, canCreate, canModify, run }: { data: EcommerceBootstrap; canCreate: boolean; canModify: boolean; run: (action: () => Promise<unknown>, success: string) => Promise<unknown | undefined> }) {
  return <div className="space-y-5 fade-up">
