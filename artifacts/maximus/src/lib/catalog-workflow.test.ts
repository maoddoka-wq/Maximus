...
test('permet de désactiver un module malgré une ancienne fonctionnalité de pack', () => {
  const data = fixture();
  updateCatalogDraft(data, draft => {
    draft.moduleStatuses.commerce = 'INACTIF';
    draft.sectorPresets = draft.sectorPresets.map(sector => ({
      ...sector,
      moduleIds: sector.moduleIds.filter(moduleId => moduleId !== 'commerce'),
      modulePackIds: Object.fromEntries(
        Object.entries(sector.modulePackIds ?? {}).filter(([moduleId]) => moduleId !== 'commerce'),
      ),
      moduleFeatures: Object.fromEntries(
        Object.entries(sector.moduleFeatures ?? {}).filter(([moduleId]) => moduleId !== 'commerce'),
      ),
    }));
    draft.moduleOverrides.commerce = {
      featurePacks: [
        {
          id: 'legacy-commerce-pack',
          name: 'Ancien pack',
          description: 'Pack historique',
          featureIds: ['devis-et-commandes'],
        },
      ],
    };
  });

  assert.deepEqual(validateCatalogDraft(data).errors, []);
  publishCatalogDraft(data);

  assert.equal(data.moduleStatuses?.commerce, 'INACTIF');
  assert.deepEqual(data.moduleOverrides?.commerce?.featurePacks, [
    {
      id: 'legacy-commerce-pack',
      name: 'Ancien pack',
      description: 'Pack historique',
